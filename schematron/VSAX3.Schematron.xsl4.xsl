...
<xsl:stylesheet version="1.0" 
    ...
    xmlns:unijsfunc="http://js.com/jsnamespace"
...
<xsl:template match="iso:schema[not(@queryBinding) or @queryBinding='xslt' 
	...
	<axsl:stylesheet>
		...
		<axsl:template name="genPath">
			<axsl:param name="prevPath"/>
			<axsl:variable name="currPath" select="concat('/',name(),'[',count(preceding-sibling::*[name() = name(current())])+1,']',$prevPath)"/>
			<axsl:for-each select="parent::*">
				<axsl:call-template name="genPath">
					<axsl:with-param name="prevPath" select="$currPath"/>
				</axsl:call-template>
			</axsl:for-each>
			<axsl:if test="not(parent::*)">
				<axsl:value-of select="$currPath"/>
			</axsl:if>
		</axsl:template>
		
		<xsl:comment>----------------------------------</xsl:comment>
		<xsl:comment>--Здесь определены переменные из VSAX3:</xsl:comment>
		<xsl:comment>----------------------------------</xsl:comment>
		<xsl:comment>$SnpVariables$</xsl:comment>
		<xsl:comment>----------------------------------</xsl:comment>
		
		<axsl:output omit-xml-declaration="yes" cdata-section-elements="Error" encoding="UTF-8"/>
...
<!-- ============================================================== -->
	...
	<xsl:template name="MyReplace">           
		   ...
		   <xsl:choose>           
			   <xsl:when test="contains($input, $from)">           
				   ...
				   <!--   вход в итерацию -->           
				   <xsl:call-template name="MyReplace">           
	...
	<xsl:template match="usch:error" mode="text">
    <axsl:message terminate="no">progress:data</axsl:message>
		<axsl:element name="Error">
			<axsl:attribute name="FullPath"><axsl:call-template name="genPath"/><xsl:if test="string-length(normalize-space(@path))>0">/<xsl:value-of select="@path"/></xsl:if></axsl:attribute>
      <axsl:attribute name="CriticalCode"><xsl:value-of select="@critical-code"/></axsl:attribute>
			<axsl:attribute name="ErrorCode"><xsl:value-of select="@code"/></axsl:attribute>
			<xsl:variable name="ValueVar"><xsl:value-of select="normalize-space(@value-of-select)"/></xsl:variable>
			<axsl:attribute name="Value"><xsl:text disable-output-escaping="yes">&lt;</xsl:text><xsl:text>xsl:value-of select="</xsl:text><xsl:if test="string-length($ValueVar)>0"><xsl:value-of select="$ValueVar"/></xsl:if><xsl:if test="string-length($ValueVar)=0">string('')</xsl:if><xsl:text disable-output-escaping="yes">"/></xsl:text></axsl:attribute>
			<axsl:attribute name="IdDocNaim"><xsl:value-of select="@doc-id-caption"/></axsl:attribute>
			<xsl:variable name="DocPathVar"><xsl:value-of select="normalize-space(@doc-id-path)"/></xsl:variable>
			<axsl:attribute name="IdDocZnach"><xsl:text disable-output-escaping="yes">&lt;</xsl:text><xsl:text>xsl:value-of select="</xsl:text><xsl:if test="string-length($DocPathVar)>0"><xsl:value-of select="$DocPathVar"/></xsl:if><xsl:if test="string-length($DocPathVar)=0">string('')</xsl:if><xsl:text disable-output-escaping="yes">"/></xsl:text></axsl:attribute>
			<xsl:apply-templates mode="text"/>
			<!--<xsl:variable name="str1">
				<xsl:call-template name="MyReplace">           
				   <xsl:with-param name="input" select="string(.)"/>           
				   <xsl:with-param name="from" select="'usch:'"/>           
...
<!-- ISO VALUE-OF -->
<!-- ============================================================== -->
<xsl:template match="usch:value-of" mode="text" >

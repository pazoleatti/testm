...
spool &1;
DECLARE
  v_task_name varchar2(128):='update ndfl_person_income';  
...
BEGIN
  ...
  dbms_output.put_line(v_task_name||'.operation_date ' || to_char(v_cnt_oper_date) || ' rows.');
  dbms_output.put_line(v_task_name||'.action_date ' || to_char(v_cnt_act_date) || ' rows.');
  dbms_output.put_line(v_task_name||'.row_type ' || to_char(v_cnt_row_type) || ' rows.');
EXCEPTION
  WHEN others THEN

...
on a.code=b.code and a.name=b.name and a.version=b.version and a.status=b.status
where a.id is null
order by b.code, b.name;

select 'Строки, которые есть в эталоне, но отсутствуют в схеме' from dual; 

select '"'||to_char(a.id)||'";"'||a.code||'";"'||a.name||'";"'||to_char(a.version,'dd.mm.yyyy')||'";"'||to_char(a.status)||'";"'||to_char(a.record_id)||'"'
from &3..REF_BOOK_OKTMO a
full outer join &2..REF_BOOK_OKTMO b 
on a.code=b.code and a.name=b.name and a.version=b.version and a.status=b.status
where b.id is null

            ...
            for (Ndfl2Node documentNode : documentNodeList) {
                ...
                for (Ndfl2Node svedDohNode : svedDohNodeList) {
                    ...
                    if (taxSum > withHoldingTax) {
                        createWarnMessage(logger, documentNode, "«Сумма налога перечисленная» рассчитана некорректно", "В \"Разделе 2. \"Общие суммы дохода и налога по итогам налогового периода\" «Сумма налога перечисленная» (\"$taxSum\") не должна превышать «Сумму налога удержанную» (\"$withHoldingTax\").")
                    }
                }
            }
        }

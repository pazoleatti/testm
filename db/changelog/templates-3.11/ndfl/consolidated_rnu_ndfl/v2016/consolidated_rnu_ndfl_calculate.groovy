...
class Calculate extends AbstractScriptClass {
    ...
    void consolidation() {
        ...
        ConsolidationSourceDataSearchFilter filter = ConsolidationSourceDataSearchFilter.builder()
                ...
                .departmentId(declarationData.getKnfType() != RefBookKnfType.FOR_APP2 ? parentTB.id : null)
        ...
        if (pickedRows.isEmpty()) {
            ...
            if (acceptedPnfIds.isEmpty() && !unacceptedPnfIds.isEmpty()) {
                ...
                logger.error("Формы, содержащие данные для включения в КНФ, существуют, но ни одна из них не находится в состоянии \"Принята\". Номера найденных ПНФ: %s", unacceptedPnfIds.join(", "))
            } else {

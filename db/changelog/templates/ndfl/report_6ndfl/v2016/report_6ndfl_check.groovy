...
// Атрибуты 2 НДФЛ
    ...
    def checkXml() {
        ...
        sumStavkaNodes.each { sumStavkaNode ->
            ScriptUtils.checkInterrupted()
            def stavka = sumStavkaNode.attributes()[ATTR_RATE] ? sumStavkaNode.attributes()[ATTR_RATE].toInteger() : 0
 ...
 */
    def getNdfl2DeclarationDataId() {
        def result = []
        List<DeclarationData> declarationDataList = declarationService.find(DECLARATION_TYPE_NDFL2_1_ID, declarationData.departmentReportPeriodId)

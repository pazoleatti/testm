...
class DepartmentConfigScript extends AbstractScriptClass {
    ...
    final int signatoryApprovalDocColIndex = 12
    ...
    class Cell {
        ...
        RefBookSignatoryMark getSignatoryMark() {
            ...
            if (!value || value.length() != 1 || !value.isNumber()) {
                ...
                } else if (RefBookSignatoryMark.TAX_AGENT_AMBASSADOR == signatoryMark.getCode()
                        && !row.cell(signatoryApprovalDocColIndex).value) {
                    logError("Если признак подписанта = 2, то соответствующая ячейка столбца " +
                            "\"Наименование документа, подтверждающего полномочия\" обязательна для заполнения.")
                }
            }
            return signatoryMark
        }

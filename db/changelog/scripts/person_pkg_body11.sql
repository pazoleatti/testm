  ...
  begin
    open v_ref for
      select person2.person_id,
             ...
             person3.version as person_version,
             person3.status as person_status,
      ...
      from (
            ...
            select /*+ use_hash(t person) parallel(t 4) parallel(person 4)*/ t.id as person_id,
              ...
              from ndfl_person t join ref_book_person person on (replace(lower(nvl(person.last_name,'empty')),' ','') = replace(lower(t.last_name),' ','') and
                                 ...
                                 join tmp_version tv on (tv.version = person.version and tv.record_id = person.record_id)
                                 left join ref_book_id_doc doc on (doc.person_id in (select id from ref_book_person where record_id=person.record_id) and doc.status=0)
                                 left join ref_book_id_tax_payer tax on (tax.person_id in (select id from ref_book_person where record_id=person.record_id) and tax.status=0)
            ...
            select /*+ use_hash(t doc) parallel(t 4) parallel(doc 4)*/ t.id as person_id,
              ...
              from ndfl_person t join ref_book_doc_type dt on (dt.code=t.id_doc_type)
                                 join ref_book_id_doc doc on (doc.doc_id=dt.id and regexp_replace(lower(doc.doc_number),'[^0-9A-Za-zА-Яа-я]','') = regexp_replace(lower(t.id_doc_number),'[^0-9A-Za-zА-Яа-я]','') and doc.status=0)
            ...
            select /*+ use_hash(t tax) parallel(t 4) parallel(tax 4)*/ t.id as person_id,
             ...
             where t.declaration_data_id=p_declaration
               ...
               join ref_book_person person3 on (person3.id=person2.ref_book_person_id and person3.status=0)
               left join ref_book_id_doc doc on (doc.id=person2.ref_book_id_doc_id and doc.status=0)
               left join ref_book_id_tax_payer tax on (tax.id=person2.book_id_tax_payer_id and tax.status=0)
               left join ref_book_person_tb tb on (tb.person_id in (select id from ref_book_person where record_id=person3.record_id) and tb.status=0)                                                
               left join ref_book_address addr on (addr.id=person3.address and addr.status=0);

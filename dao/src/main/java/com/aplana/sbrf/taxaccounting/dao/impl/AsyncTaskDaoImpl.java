...
public class AsyncTaskDaoImpl extends AbstractDao implements AsyncTaskDao {
    ...
    @Override
    public AsyncTaskData reserveTask(String node, String priorityNode, int timeoutHours, AsyncQueue queue, int maxTasksPerNode) {
        MapSqlParameterSource params = new MapSqlParameterSource();
        ...
        try {
            String sql = "select id from async_task where id = (select id from (\n" +
                    ...
                    "  queue = :queue and (node is null or current_timestamp > start_process_date + interval '" + timeoutHours + "' hour) and \n" +
            ...
            if (rowsUpdated == 1) {
                AsyncTaskData asyncTaskData = findById(lockedAsyncId);

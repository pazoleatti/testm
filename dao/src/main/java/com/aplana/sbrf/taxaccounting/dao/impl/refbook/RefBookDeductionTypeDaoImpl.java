...
public class RefBookDeductionTypeDaoImpl extends AbstractDao implements RefBookDeductionTypeDao {
    ...
    public List<RefBookDeductionType> findAll() {
        return getJdbcTemplate().query("" +
                        ...
                        "where t.status = 0 and t.version <= sysdate and (t.version_end is null or sysdate <= t.version_end) \n" +
                        "and m.status = 0 and m.version <= sysdate and (m.version_end is null or sysdate <= m.version_end)",

...
public class DeclarationDataDaoImpl extends AbstractDao implements DeclarationDataDao {
    ...
    @Override
    public List<Long> findIdsByFilter(DeclarationDataFilter filter, DeclarationDataSearchOrdering ordering, Order order){
        BooleanBuilder where = new BooleanBuilder();
        ...
        //Определяем способ сортировки
        String orderingProperty = ordering.name();


        OrderSpecifier orderingSpecifier = QueryDSLOrderingUtils.getOrderSpecifierByPropertyAndOrder(
                dataJournalItemQBean, orderingProperty, order, declarationData.id.desc());
        ...
        return sqlQueryFactory.select()
                ...
                .leftJoin(declarationData.declDataDocStateFk, refBookDocState)
                .innerJoin(departmentFullpath).on(departmentFullpath.id.eq(department.id))
                .orderBy(orderingSpecifier)
                .where(where)
                .transform(GroupBy.groupBy(declarationData.id).list(declarationData.id));

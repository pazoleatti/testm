...
/**
 * TextArea для редактирования текста в ячейке.
 * Аналог com.google.gwt.cell.client.EditTextCell без привязки к нажатиям ENTER и ESCAPE с коммитом на BLUR
 */
public class EditTextAreaCell extends AbstractEditableCell<String, EditTextAreaCell.ViewData> {

    interface Template extends SafeHtmlTemplates {
        @Template("<textarea rows=\"3\" tabindex=\"-1\" wrap=\"off\">{0}</textarea>")
    ...
    public void render(Context context, String value, SafeHtmlBuilder sb) {
        ...
        if (toRender != null && toRender.trim().length() > 0) {
            ...
            sb.append(SafeHtmlUtils.fromTrustedString("<div style=\"white-space: pre\">" + renderer.render(toRender).asString() + "</div>"));
        } else {
      /*

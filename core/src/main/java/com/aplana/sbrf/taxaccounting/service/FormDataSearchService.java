...
public interface FormDataSearchService {
	 ...
	 * @deprecated - нужно использовать {@link #getAvailableDepartments(int, TaxType)}
	 */
	List<Department> listAllDepartmentsByParentDepartmentId(int parentDepartmentId);
	
	/**
	 * Получить список подразделений, к налоговым формам которых имеет доступ пользователь
	 * @param userId идентификатор пользователя
	 * @param taxType вид налога
	 * @return список подразделений, к налоговым формам которых имеет доступ пользователь, или null, 
	 * если пользователь имеет доступ к налоговым формам во всех подразделениям (Контролёр УНП)
	 */
	List<Department> getAvailableDepartments(int userId, TaxType taxType);

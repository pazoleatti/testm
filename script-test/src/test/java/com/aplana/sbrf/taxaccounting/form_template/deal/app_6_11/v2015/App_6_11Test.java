...
public class App_6_11Test extends ScriptTestBase {
    ...
    public void check1Test() throws ParseException {
        ...
        int i = 0;
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Дата сделки (поставки)"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Наименование контрагента и ОПФ"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Сумма сделки (с учетом НКД), в валюте расчетов"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Валюта расчетов по сделке"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Сумма сделки (с учетом НКД), руб."), entries.get(i++).getMessage());
        ...
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Регистрационный код ценной бумаги"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Количество бумаг по сделке, шт."), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Цена за 1 шт., руб."), entries.get(i++).getMessage());
        ...
        entries = testHelper.getLogger().getEntries();
        i = 0;
        Assert.assertEquals("Строка 1: Значение графы «Количество бумаг по сделке, шт.» должно быть больше «0»!", entries.get(i++).getMessage());
        Assert.assertEquals(i, testHelper.getLogger().getEntries().size());
        testHelper.getLogger().clear();

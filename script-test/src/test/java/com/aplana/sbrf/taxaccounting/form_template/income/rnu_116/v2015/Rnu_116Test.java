package com.aplana.sbrf.taxaccounting.form_template.income.rnu_116.v2015;
 ...
 * РНУ 116
 */
public class Rnu_116Test extends ScriptTestBase {
    private static final int TYPE_ID = 844;
    ...
    protected ScriptTestMockHelper getMockHelper() {
        return getDefaultScriptTestMockHelper(Rnu_116Test.class);
    }
    ...
    public void check1Test() throws ParseException {
        ...
        formData.initFormTemplateParams(testHelper.getTemplate("..//src/main//resources//form_template//income//rnu_116//v2015//"));
        ...
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Вид сделки"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Дата заключения сделки"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Дата исполнения сделки"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Контрагент"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Тип сделки"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Код валюты по сделке приобретения (требования)"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Объем покупаемой валюты"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Код валюты по сделке продажи (обязательства)"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Объем продаваемой валюты"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Курс сделки"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Курс Банка России на дату исполнения (досрочного исполнения) сделки, руб. В отношении требования"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Курс Банка России на дату исполнения (досрочного исполнения) сделки, руб. В отношении обязательства"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Требования (обязательства) по сделке, руб. Требования"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Требования (обязательства) по сделке, руб. Обязательства"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Доходы (расходы) учитываемые в целях налога на прибыль по сделке, руб. Доходы"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Доходы (расходы) учитываемые в целях налога на прибыль по сделке, руб. Расходы"), entries.get(i++).getMessage());

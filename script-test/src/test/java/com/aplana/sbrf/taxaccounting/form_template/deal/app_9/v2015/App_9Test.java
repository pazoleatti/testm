...
public class App_9Test extends ScriptTestBase {
    ...
    static {
        formTypeIdByTemplateIdMap.put(853, 853); // Журнал взаиморасчетов по ТБ
    ...
    static {
        templatesPathMap.put(853, "..//src/main//resources//form_template//deal//journal_settlements_tb//v2015//");
    ...
    public void calc1Test() {
        ...
        String [] nonEmptyColumns = { "name", "sum4", "sum42", "sum43", "sum44", "sum45", "sum46",
                "sum51", "sum52", "sum53", "sum54", "sum55", "sum56", "sum6" };
        ...
        checkLogger();
        // графа 16 (6)
        Assert.assertEquals(12L, row.getCell("sum6").getNumericValue().longValue());
    }
    ...
    public void check1Test() throws ParseException {
        ...
        ReportPeriod oldPeriod = testHelper.getReportPeriodService().get(1);
        ...
        testHelper.execute(FormDataEvent.CHECK);
        i = 0;
        msg = String.format("Строка %s: Организация «%s» не является взаимозависимым лицом в данном отчетном периоде!", rowIndex, "D");
        Assert.assertEquals(msg, entries.get(i++).getMessage());
        Assert.assertEquals(i, testHelper.getLogger().getEntries().size());
        testHelper.getLogger().clear();
        when(testHelper.getReportPeriodService().get(anyInt())).thenReturn(oldPeriod);

        // 3. Проверка на отсутствие в списке не ВЗЛ ОРН (9 месяцев)
        reportPeriod.setOrder(3);
        when(testHelper.getReportPeriodService().get(anyInt())).thenReturn(reportPeriod);
        for (String column : nonEmptyColumns) {
            row.getCell(column).setValue(1L, rowIndex);
        }
        ...
        testHelper.execute(FormDataEvent.CHECK);
        i = 0;
        msg = String.format("Строка %s: Организация «%s» не является взаимозависимым лицом с общим режимом налогообложения в данном отчетном периоде!", rowIndex, "D");
        Assert.assertEquals(msg, entries.get(i++).getMessage());
        Assert.assertEquals(i, testHelper.getLogger().getEntries().size());
        testHelper.getLogger().clear();
        when(testHelper.getReportPeriodService().get(anyInt())).thenReturn(oldPeriod);
    ...
    private List<DataRow<Cell>> getFillDataRows(int sourceTemplateId, FormData sourceFormData) {
        ...
        switch (sourceTemplateId) {
            case 853 : // Журнал взаиморасчетов по ТБ
                row = sourceFormData.createDataRow();
                row.getCell("statReportId").setValue(testRefbookId, null);
    ...
    void checkData(List<DataRow<Cell>> dataRows) {
        ...
        // графа 3  (3) - зависимая
        // графа 4  (4.1)
        Assert.assertEquals(0L, row.getCell("sum4").getNumericValue().longValue());
        // графа 5  (4.2)
        Assert.assertEquals(0L, row.getCell("sum42").getNumericValue().longValue());
        // графа 6  (4.3)
        Assert.assertEquals(0L, row.getCell("sum43").getNumericValue().longValue());
        // графа 7  (4.4)
        Assert.assertEquals(100000L, row.getCell("sum44").getNumericValue().longValue());
        // графа 8  (4.5)
        Assert.assertEquals(100L, row.getCell("sum45").getNumericValue().longValue());
        // графа 9  (4.6)
        Assert.assertEquals(100L, row.getCell("sum46").getNumericValue().longValue());
        // графа 10 (5.1)
        ...
        // графа 13 (5.4)
        Assert.assertEquals(0L, row.getCell("sum54").getNumericValue().longValue());
        // графа 14 (5.5) +
        Assert.assertEquals(0L, row.getCell("sum55").getNumericValue().longValue());
        // графа 15 (5.6)
        Assert.assertEquals(0L, row.getCell("sum56").getNumericValue().longValue());
        // графа 16 (6)
        String [] calc16Aliases = { "sum4", "sum42", "sum43", "sum44", "sum45", "sum46", "sum51", "sum52", "sum53", "sum54", "sum55", "sum56" };
        long expected16 = 0L;
        for (String alias : calc16Aliases) {
            expected16 += row.getCell(alias).getNumericValue().longValue();
        }
        Assert.assertEquals(expected16, row.getCell("sum6").getNumericValue().longValue());
    }

    private void mockProviders() {
        long [] refBookIds = { 511L, 520L, 525L };

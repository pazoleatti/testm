...
@Ignore
public class Report2NdflScriptTest extends DeclarationScriptTestBase {
    ...
    protected ScriptTestMockHelper getMockHelper() {
        return getDefaultScriptTestMockHelper(Report2NdflScriptTest.class);
    }
    ...
    private boolean validateResultBySchema(String xml) throws SAXException, UnsupportedEncodingException {
        ...
        v.useXMLSchema(true);
        v.setJAXP12SchemaSource(Report2NdflScriptTest.class.getResourceAsStream("/com/aplana/sbrf/taxaccounting/form_template/ndfl/report_2ndfl_1/v2016/schema.xsd"));
        return v.isValid();
    }
    ...
    private List<NdflPerson> createNdflPersonMocks(LocalDateTime taxDate, LocalDateTime paymentDate) {
        ...
        when(testHelper.getNdflPersonService().findDeductionsWithDeductionsMarkNotOstalnie(anyLong(), any(Date.class), any(Date.class), anyBoolean())).thenReturn(ndflPerson.getDeductions());
        when(testHelper.getNdflPersonService().findPrepaymentsByPeriodAndNdflPersonId(anyLong(), anyInt(), any(Date.class), any(Date.class), anyBoolean())).thenReturn(ndflPerson.getPrepayments());

...
public class CHDTest extends ScriptTestBase {
    ...
    public void check1Test() throws ParseException {
        ...
        formData.initFormTemplateParams(testHelper.getTemplate("..//src/main//resources//form_template//market//chd//v2016//"));
        ...
        Assert.assertEquals("Строка 1: Значение графы «Дата выдачи (дд.мм.гг.)» должно быть больше либо равно значения графы «Дата кредитного договора (дд.мм.гг.)»!", entries.get(i++).getMessage());
        Assert.assertEquals("Строка 1: Значение графы «Дата планируемого погашения с учетом последней пролонгации (дд.мм.гг.)» должно быть больше либо равно значения графы «Дата кредитного договора (дд.мм.гг.)»!", entries.get(i++).getMessage());
        Assert.assertEquals("Строка 1: Значение графы «Дата планируемого погашения с учетом последней пролонгации (дд.мм.гг.)» должно быть больше либо равно значения графы «Дата выдачи (дд.мм.гг.)»!", entries.get(i++).getMessage());
        Assert.assertEquals(i, testHelper.getLogger().getEntries().size());
        testHelper.getLogger().clear();

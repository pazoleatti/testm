package com.aplana.sbrf.taxaccounting.form_template.vat.vat_724_6.v2014;
...
/**
 * (724.6) Отчёт о суммах НДС начисленных налоговым агентом с сумм дохода иностранных юридических лиц (балансовый счёт 60309.02)
 */
public class Vat_724_6Test extends ScriptTestBase {
    private static final int TYPE_ID = 604;
    ...
    protected ScriptTestMockHelper getMockHelper() {
        return getDefaultScriptTestMockHelper(Vat_724_6Test.class);
    }
    ...
    void checkLoadData(List<DataRow<Cell>> dataRows) {
        ...
        for (DataRow<Cell> row : dataRows) {
            ...
            // графа 3
            Assert.assertNotNull("row.contragent[" + row.getIndex() + "]", row.getCell("contragent").getStringValue());

            // графа 4
            Assert.assertNotNull("row.type[" + row.getIndex() + "]", row.getCell("type").getStringValue());
            ...
            // графа 6
            Assert.assertNotNull("row.number[" + row.getIndex() + "]", row.getCell("number").getStringValue());
            ...
            // графа 8
            Assert.assertNotNull("row.date[" + row.getIndex() + "]", row.getCell("date").getDateValue());

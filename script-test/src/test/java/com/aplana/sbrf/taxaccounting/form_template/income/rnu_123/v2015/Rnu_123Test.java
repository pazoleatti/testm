...
public class Rnu_123Test extends ScriptTestBase {
    ...
    public void check1Test() throws ParseException {
        ...
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Сумма гарантии / аккредитива, (вал.)"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Валюта"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Дата операции. Фактическое отражение в бухгалтерском учете"), entries.get(i++).getMessage());
        Assert.assertEquals(String.format(ScriptUtils.WRONG_NON_EMPTY, 1, "Курс Банка России (руб.). На дату фактического отражения в бухгалтерском учете"), entries.get(i++).getMessage());

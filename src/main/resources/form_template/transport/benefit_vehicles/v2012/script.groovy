package form_template.transport.benefit_vehicles.v2012
...
void importData() {
    ...
    def headerMapping = [
            ...
            (xml.row[0].cell[4]): 'Мощность',
            (xml.row[0].cell[6]): 'Код налоговой льготы',
            (xml.row[0].cell[7]): 'Использование льготы',
            (xml.row[1].cell[4]): 'Величина',
            (xml.row[1].cell[5]): 'Ед. измерения',
...
void addTransportData(def xml) {
    ...
    for (def row : xml.row) {
        ...
        // графа 2
        newRow.codeOKATO = getRecordIdImport(96, 'CODE', row.cell[xmlIndexCol].text(), rnuIndexRow, xmlIndexCol + colOffset)
        xmlIndexCol++
        ...
        // графа 7
        newRow.taxBenefitCode = getRecordIdImport(6, 'CODE', row.cell[xmlIndexCol].text(), rnuIndexRow, xmlIndexCol + colOffset)
        xmlIndexCol++

package form_template.transport.declaration.v2016
...
def generateXML() {
    ...
    builder.Файл(
            ...
            ВерсФорм : departmentParam?.FORMAT_VERSION?.value) {
                ...
                СумНалПУ(КБК : "18210604011021000110") {
                    dataRowsMap.each { totalRow, groupRows ->
                        СумПУ(
                            ...
                            // Раздел 2
                            groupRows.each { row ->
                                def codVichet = getRefBookValue(6L, row.deductionCode)?.CODE?.value
                                РасчНалТС(
                                        [
                                                ...
                                                ДатаРегТС : row.regDate?.format('dd.MM.yyyy')
                                        ] +
                                                (row.regDateEnd ? [ДатаСнРегТС: row.regDateEnd?.format('dd.MM.yyyy')] : [:]) +
                                                [НалБаза  : row.taxBase] +
                                                ...
                                                [
                                                        ВыпускТС : row.years,
                                                        ГодВыпТС : row.createYear?.format('yyyy'),
                                                ...
                                                (codVichet != null ? [КодВычет : codVichet] : [:]) +
                                                (codVichet == '40200' ? [СумВычет : getLong(row.deductionSum)] : [:]) +

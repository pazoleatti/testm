...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[1][4]) : tmpRow.getCell('regCountryCode').column.name]),
            ...
            ([(headerRows[1][10]): tmpRow.getCell('outcome').column.name]),
...
def getNewRow(String[] rowCells, def columnCount, def fileRowIndex, def rowIndex, def isTotal) {
    ...
    if (!isTotal) {
        ...
        // графа 5
        if (countryMap != null) {
            formDataService.checkReferenceValue(pure(rowCells[5]), [countryMap.CODE?.stringValue], getColumnName(newRow, 'regCountryCode'), map.NAME.value, fileRowIndex, 5 + colOffset, logger, false)
        }
        ...
        // графа 11
        newRow.outcome = parseNumber(pure(rowCells[11]), fileRowIndex, 11 + colOffset, logger, true)

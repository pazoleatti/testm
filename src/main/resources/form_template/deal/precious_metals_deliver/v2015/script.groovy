...
void calc() {
    ...
    for (row in dataRows) {
        ...
        // Цена (тариф) за единицу измерения без учета НДС, руб.
        row.priceOne = null
        if (row.incomeSum && !row.consumptionSum) {
            row.priceOne = row.incomeSum
        } else if (row.consumptionSum && !row.incomeSum) {
            row.priceOne = row.consumptionSum
        } else if (row.consumptionSum && row.incomeSum) {
            row.priceOne = (row.incomeSum - row.consumptionSum).abs()
        }
...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[1][3]) : tmpRow.getCell('dependence').column.name]),
            ([(headerRows[1][4]) : tmpRow.getCell('innKio').column.name]),
            ([(headerRows[1][5]) : tmpRow.getCell('country').column.name]),
            ([(headerRows[1][6]) : tmpRow.getCell('countryCode1').column.name]),
            ([(headerRows[1][7]) : tmpRow.getCell('contractNum').column.name]),
            ([(headerRows[1][8]) : tmpRow.getCell('contractDate').column.name]),
            ([(headerRows[1][9]) : tmpRow.getCell('transactionNum').column.name]),
            ([(headerRows[1][10]): tmpRow.getCell('dealType').column.name]),
            ([(headerRows[1][11]): tmpRow.getCell('transactionDeliveryDate').column.name]),
            ([(headerRows[2][12]): tmpRow.getCell('innerCode').column.name]),
            ([(headerRows[2][13]): tmpRow.getCell('unitCountryCode').column.name]),
            ([(headerRows[2][14]): tmpRow.getCell('signPhis').column.name]),
            ([(headerRows[2][15]): tmpRow.getCell('signTransaction').column.name]),
            ([(headerRows[2][16]): tmpRow.getCell('countryCode2').column.name]),
            ([(headerRows[2][17]): tmpRow.getCell('region1').column.name]),
            ([(headerRows[2][18]): tmpRow.getCell('city1').column.name]),
            ([(headerRows[2][19]): tmpRow.getCell('settlement1').column.name]),
            ([(headerRows[2][20]): tmpRow.getCell('countryCode3').column.name]),
            ([(headerRows[2][21]): tmpRow.getCell('region2').column.name]),
            ([(headerRows[2][22]): tmpRow.getCell('city2').column.name]),
            ([(headerRows[2][23]): tmpRow.getCell('settlement2').column.name]),
            ([(headerRows[1][24]): tmpRow.getCell('conditionCode').column.name]),
            ([(headerRows[1][25]): tmpRow.getCell('count').column.name]),
            ([(headerRows[1][26]): tmpRow.getCell('incomeSum').column.name]),
            ([(headerRows[1][27]): tmpRow.getCell('consumptionSum').column.name]),
            ([(headerRows[1][28]): tmpRow.getCell('priceOne').column.name]),
            ([(headerRows[1][29]): tmpRow.getCell('totalNds').column.name]),
            ([(headerRows[1][31]): tmpRow.getCell('transactionDate').column.name]),
            ([(headerRows[3][0]) : 'гр. 1']),

...
@Field
def allColumns = ['rowNumber', 'name', 'iksr', 'countryCode', 'sum', 'docNumber', 'docDate', 'price', 'cost', 'dealDoneDate']
...
void logicCheck() {
    ...
    for (row in dataRows) {
        ...
        // 4. Проверка возможности заполнения цены и стоимости
        if (row.sum == null) {
...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[1][7]) : tmpRow.getCell('price').column.name]),
            ([(headerRows[1][8]) : tmpRow.getCell('cost').column.name]),
            ([(headerRows[1][9]): tmpRow.getCell('dealDoneDate').column.name])
    ]
    (1..10).each {

...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[1][9]): tmpRow.getCell('currencyCode').column.name]),
            ([(headerRows[1][10]): tmpRow.getCell('dealCountryCode').column.name]),
            ([(headerRows[1][11]): tmpRow.getCell('income').column.name]),
            ([(headerRows[1][12]): tmpRow.getCell('outcome').column.name]),
            ([(headerRows[1][13]): tmpRow.getCell('price').column.name]),
            ([(headerRows[1][14]): tmpRow.getCell('cost').column.name]),
            ([(headerRows[1][15]): tmpRow.getCell('dealDoneDate').column.name]),
            ([(headerRows[2][0]) : 'гр. 1']),
            ([(headerRows[2][1]) : 'гр. 2']),
            ([(headerRows[2][2]) : 'гр. 3']),

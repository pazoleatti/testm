...
@Field
def allColumns = ['rowNumber', 'name', 'iksr', 'countryName', 'countryCode', 'serviceName', 'sum', 'docNumber',
...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[1][5]): tmpRow.getCell('serviceName').column.name]),
            ([(headerRows[1][6]): tmpRow.getCell('sum').column.name]),
...
def getNewRowFromXls(def values, def colOffset, def fileRowIndex, def rowIndex) {
    ...
    // графа 5
    newRow.serviceName = getRecordIdImport(13, 'CODE', values[colIndex], fileRowIndex, colIndex + colOffset, false)
    colIndex++

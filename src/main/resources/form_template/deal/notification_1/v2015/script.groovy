package form_template.deal.notification_1.v2015
 ...
 *
 * declarationTypeId = 23
 * declarationTemplateId = 21007
 *
...
@Field
def notification1Type = 23

@Field
def versionFormat = '5.01'
                    ...
                    for (row in dataRowHelper.allSaved) {
                        ...
                        СвКонтрСд(
                            ...
                            if (row.organName != null) {
                                def map = getRefBookValue(520, row.organName)
                                organInfo = getRefBookValue(513, map.ORG_CODE.referenceValue)?.CODE?.numberValue
                                organName = map?.NAME?.stringValue
                                organINN = map?.INNKIO?.stringValue
                                organKPP = map?.KPP?.stringValue
                                organRegNum = map?.RS?.stringValue
                                taxpayerCode = map?.TAX_CODE_INCORPORATION?.stringValue
...
List<String> getErrorVersion(record) {
    List<String> errorList = new ArrayList<String>()
    if (record.FORMAT_VERSION.stringValue == null || !record.FORMAT_VERSION.stringValue.equals(versionFormat)) {

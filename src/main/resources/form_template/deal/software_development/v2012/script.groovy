package form_template.deal.software_development.v2012
...
void logicCheck() {
    ...
    for (row in dataRowHelper.getAllCached()) {
        ...
        checkNSI(row, "serviceType", "Услуги в части программного обеспечения",11)
    }
}
...
def addData(def xml) {
    ...
    for (def row : xml.row) {
        ...
        // графа 5
        newRow.expensesSum = getNumber(row.cell[indexCell].text(), indexRow, indexCell)
        indexCell++
        ...
        // графа 8
        newRow.serviceType = getRecordId(11, 'CODE', row.cell[indexCell].text(), date, cache, indexRow, indexCell, false)
        indexCell++

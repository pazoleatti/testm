package form_template.deal.software_development.v1970
...
@Field
def nonEmptyColumns = ['rowNumber', 'fullNamePerson', 'countryCode', 'expensesSum', 'docNumber', 'docDate',
...
void importData() {
    ...
    def headerMapping = [
            ...
            (xml.row[0].cell[3]): getColumnName(tmpRow, 'expensesSum'),
            (xml.row[0].cell[4]): getColumnName(tmpRow, 'docNumber'),
            (xml.row[0].cell[5]): getColumnName(tmpRow, 'docDate'),
            (xml.row[0].cell[6]): getColumnName(tmpRow, 'serviceType'),
...
void addData(def xml, int headRowCount) {
    ...
    for (def row : xml.row) {
        ...
        // графа 8
        newRow.serviceType = getRecordIdImport(11, 'CODE', row.cell[xmlIndexCol].text(), xlsIndexRow, xmlIndexCol + colOffset)
        xmlIndexCol++

...
@Field
def nonEmptyColumns = ['dealDate', 'name', 'currencySum', 'currencyCode', 'sum', 'docNumber', 'docDate',
...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[2][0]): tmpRow.getCell('rowNumber').column.name]),
            ([(headerRows[2][1]): 'Сокращенная форма']),
            ([(headerRows[3][1]): 'Данные для расчета сумм доходов по сделкам']),
            ([(headerRows[4][1]): tmpRow.getCell('dealDate').column.name]),
            ([(headerRows[4][2]): tmpRow.getCell('name').column.name]),
            ([(headerRows[4][3]): tmpRow.getCell('dealMode').column.name]),
            ([(headerRows[4][4]): tmpRow.getCell('iksr').column.name]),
            ([(headerRows[4][5]): tmpRow.getCell('countryName').column.name]),
            ([(headerRows[4][6]): tmpRow.getCell('countryCode').column.name]),
            ([(headerRows[4][7]): tmpRow.getCell('currencySum').column.name]),
            ([(headerRows[4][8]): tmpRow.getCell('currencyCode').column.name]),
            ([(headerRows[4][9]): tmpRow.getCell('courseCB').column.name]),
            ([(headerRows[4][10]): tmpRow.getCell('sum').column.name]),
            ([(headerRows[4][11]): tmpRow.getCell('docNumber').column.name]),
            ([(headerRows[4][12]): tmpRow.getCell('docDate').column.name]),
            ([(headerRows[4][14]): tmpRow.getCell('bondRegCode').column.name]),
            ([(headerRows[4][15]): tmpRow.getCell('count').column.name]),
            ([(headerRows[4][16]): tmpRow.getCell('price').column.name]),

...
//        newRow.countryCode =
        ...
        // графа 5
        newRow.dealSign = getNumber(row.cell[indexCell].text(), indexRow, indexCell)
        indexCell++
        ...
        // графа 11
        newRow.okeiCode = getNumber(row.cell[indexCell].text(), indexRow, indexCell)
        indexCell++
...
def getRecordId(def ref_id, String code, String value, Date date, def cache, int indexRow, int indexCell) {
    ...
    throw new Exception("Строка ${indexRow + 2} столбец ${indexCell} содержит значение, отсутствующее в справочнике!")
}
...
def getDate(def value, int indexRow, int indexCell) {
    ...
    try {
        ...
        throw new Exception("Строка ${indexRow + 14} столбец ${indexCell} содержит недопустимый тип данных!")
    }
}
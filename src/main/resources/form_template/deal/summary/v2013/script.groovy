...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[2][11]): tmpRow.getCell('similarDealGroup').column.name]),
            ([(headerRows[2][12]): tmpRow.getCell('dealNameCode').column.name]),
            ([(headerRows[2][13]): tmpRow.getCell('taxpayerSideCode').column.name]),
            ([(headerRows[2][14]): tmpRow.getCell('dealPriceSign').column.name]),
            ([(headerRows[2][15]): tmpRow.getCell('dealPriceCode').column.name]),
            ([(headerRows[2][16]): tmpRow.getCell('dealMemberCount').column.name]),
            ([(headerRows[2][17]): tmpRow.getCell('income').column.name]),
            ([(headerRows[2][18]): tmpRow.getCell('incomeIncludingRegulation').column.name]),
            ([(headerRows[2][19]): tmpRow.getCell('outcome').column.name]),
            ([(headerRows[2][20]): tmpRow.getCell('outcomeIncludingRegulation').column.name]),
            ([(headerRows[2][21]): tmpRow.getCell('dealNum2').column.name]),
            ([(headerRows[2][22]): tmpRow.getCell('dealType').column.name]),
            ([(headerRows[2][23]): tmpRow.getCell('dealSubjectName').column.name]),
            ([(headerRows[2][24]): tmpRow.getCell('dealSubjectCode1').column.name]),
            ([(headerRows[2][25]): tmpRow.getCell('dealSubjectCode2').column.name]),
            ([(headerRows[2][26]): tmpRow.getCell('dealSubjectCode3').column.name]),
            ([(headerRows[2][27]): tmpRow.getCell('otherNum').column.name]),
            ([(headerRows[2][28]): tmpRow.getCell('contractNum').column.name]),
            ([(headerRows[2][29]): tmpRow.getCell('contractDate').column.name]),
            ([(headerRows[2][30]): tmpRow.getCell('countryCode').column.name]),
            ...
            ([(headerRows[2][39]): tmpRow.getCell('deliveryCode').column.name]),
            ([(headerRows[2][40]): tmpRow.getCell('okeiCode').column.name]),
            ([(headerRows[2][41]): tmpRow.getCell('count').column.name]),
            ([(headerRows[2][42]): tmpRow.getCell('price').column.name]),
            ([(headerRows[2][43]): tmpRow.getCell('total').column.name]),
            ([(headerRows[2][44]): tmpRow.getCell('dealDoneDate').column.name]),
            ([(headerRows[2][45]): tmpRow.getCell('dealNum3').column.name]),
            ([(headerRows[2][46]): tmpRow.getCell('dealMemberNum').column.name]),
            ([(headerRows[2][47]): tmpRow.getCell('organInfo').column.name]),
            ([(headerRows[2][48]): tmpRow.getCell('countryCode3').column.name]),
            ([(headerRows[2][49]): tmpRow.getCell('organName').column.name]),
            ([(headerRows[2][50]): tmpRow.getCell('organINN').column.name]),
            ([(headerRows[2][51]): tmpRow.getCell('organKPP').column.name]),
            ([(headerRows[2][52]): tmpRow.getCell('organRegNum').column.name]),
            ([(headerRows[2][53]): tmpRow.getCell('taxpayerCode').column.name]),
            ([(headerRows[2][54]): tmpRow.getCell('address').column.name]),
            ([(headerRows[3][1]) : tmpRow.getCell('interdependenceSing').column.name]),
            ([(headerRows[3][2]) : tmpRow.getCell('f121').column.name]),
            ([(headerRows[3][3]) : tmpRow.getCell('f122').column.name]),
            ([(headerRows[3][4]) : tmpRow.getCell('f123').column.name]),
            ([(headerRows[3][5]) : tmpRow.getCell('f124').column.name]),
            ([(headerRows[3][6]) : tmpRow.getCell('f131').column.name]),
            ([(headerRows[3][7]) : tmpRow.getCell('f132').column.name]),
            ([(headerRows[3][8]) : tmpRow.getCell('f133').column.name]),
            ([(headerRows[3][9]) : tmpRow.getCell('f134').column.name]),
            ([(headerRows[3][10]): tmpRow.getCell('f135').column.name]),
            ([(headerRows[3][31]): tmpRow.getCell('countryCode1').column.name]),
            ([(headerRows[3][32]): tmpRow.getCell('region1').column.name]),
            ([(headerRows[3][33]): tmpRow.getCell('city1').column.name]),
            ([(headerRows[3][34]): tmpRow.getCell('locality1').column.name]),
            ([(headerRows[3][35]): tmpRow.getCell('countryCode2').column.name]),
            ([(headerRows[3][36]): tmpRow.getCell('region2').column.name]),
            ([(headerRows[3][37]): tmpRow.getCell('city2').column.name]),
            ([(headerRows[3][38]): tmpRow.getCell('locality2').column.name])
    ]
...
def getNewRowFromXls(def values, def colOffset, def fileRowIndex, def rowIndex) {
    ...
    // TODO (Ramil Timerbaev) КОСТЫЛЬ! если не найдена запись, то подменять ковычки на "«" и на "»"
    def tmp = values[colIndex]
    if (!newRow.organName && tmp != null && tmp.contains('"')) {
        while (tmp.contains('"')) {
            tmp = tmp.replaceFirst('"', '«')
            tmp = tmp.replaceFirst('"', '»')
        }
        newRow.organName = getRecordIdImport(9, 'NAME', tmp, fileRowIndex, colIndex + colOffset)
    }
    // TODO (Ramil Timerbaev) КОСТЫЛЬ! если не найдена запись, то добавляет в конце строки пробелы
    if (!newRow.organName) {
        for (def i = 1; i <= 2; i++) {
            tmp = tmp + ' '
            newRow.organName = getRecordIdImport(9, 'NAME', tmp, fileRowIndex, colIndex + colOffset)

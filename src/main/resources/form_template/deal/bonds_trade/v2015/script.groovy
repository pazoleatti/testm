...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[2][0]): tmpRow.getCell('rowNum').column.name]),
            ([(headerRows[2][1]): 'Сокращенная форма']),
            ([(headerRows[3][1]): 'Данные для расчета сумм доходов по сделкам']),
            ([(headerRows[4][1]): tmpRow.getCell('transactionDeliveryDate').column.name]),
            ([(headerRows[4][2]): tmpRow.getCell('contraName').column.name]),
            ([(headerRows[4][3]): tmpRow.getCell('transactionMode').column.name]),
            ([(headerRows[4][4]): tmpRow.getCell('innKio').column.name]),
            ([(headerRows[4][5]): tmpRow.getCell('contraCountry').column.name]),
            ([(headerRows[4][6]): tmpRow.getCell('contraCountryCode').column.name]),
            ([(headerRows[4][7]): tmpRow.getCell('transactionSumCurrency').column.name]),
            ([(headerRows[4][8]): tmpRow.getCell('currency').column.name]),
            ([(headerRows[4][9]): tmpRow.getCell('courseCB').column.name]),
            ([(headerRows[4][10]): tmpRow.getCell('transactionSumRub').column.name]),
            ([(headerRows[4][11]): tmpRow.getCell('contractNum').column.name]),
            ([(headerRows[4][12]): tmpRow.getCell('contractDate').column.name]),
            ([(headerRows[4][13]): tmpRow.getCell('transactionDate').column.name]),
            ([(headerRows[4][14]): tmpRow.getCell('bondRegCode').column.name]),
            ([(headerRows[4][15]): tmpRow.getCell('bondCount').column.name]),
            ([(headerRows[4][16]): tmpRow.getCell('priceOne').column.name]),
            ([(headerRows[4][17]): tmpRow.getCell('transactionType').column.name]),
            ([(headerRows[5][0]): 'гр. 1']),
            ([(headerRows[5][1]): 'гр. 2']),
            ([(headerRows[5][2]): 'гр. 3']),
            ([(headerRows[5][3]): 'гр. 4']),

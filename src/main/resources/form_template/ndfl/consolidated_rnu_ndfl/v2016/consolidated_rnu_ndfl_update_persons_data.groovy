...
class UpdatePersonsData extends AbstractScriptClass {
    ...
    void doUpdate() {
        ...
        logger.info("Завершено обновление ФЛ для формы %s из %s. Обновлено %s записей.",
                declarationData.id,
                department.getName(),
    ...
    boolean hasDul(long personId) {
        ...
        PagingResult<Map<String, RefBookValue>> dulList = provider.getRecords(getReportPeriodEndDate(), null, "INC_REP = 0 AND PERSON_ID = ${personId}".toString(), null)

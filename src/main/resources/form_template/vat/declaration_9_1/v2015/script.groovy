...
void preCalcCheck() {
    ...
    if (formData) {
        ...
        if (!codesDeclaration.isEmpty()) {
            msg = "Т.к. не найдена требуемая строка формы-источника 724.1.1, для заполнения строк %s раздела 9.1 определена декларация-источник. Вид: «%s», Подразделение: «%s», Период: «%s»%s."

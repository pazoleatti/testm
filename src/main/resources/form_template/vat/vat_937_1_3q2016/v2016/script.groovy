package form_template.vat.vat_937_1_3q2016.v2016
...
void logicCheck() {
    ...
    for (def row : dataRows) {
        ...
        def typeCode = getRefBookValue(650, row.typeCode)?.CODE?.value
        if (typeCode && typeCode.matches("^[0-9]{2}\$") && Integer.valueOf(typeCode) in ((01..05) + (07..13)) && row.salesmanInnKpp == null){
...
def getNewRow(String[] rowCells, def columnCount, def fileRowIndex, def rowIndex) {
    ...
    colIndex++
    newRow.typeCode = getRecordIdImport(650, 'CODE', pure(rowCells[colIndex]), fileRowIndex, colIndex + colOffset, false)

    // графа 3..7
    ['invoice', 'invoiceCorrecting', 'invoiceCorrection', 'invoiceCorrectingCorrection', 'documentPay'].each { alias ->
        colIndex++
...
def getNewRowFromXls(def values, def colOffset, def fileRowIndex, def rowIndex) {
    ...
    colIndex++
    newRow.typeCode = getRecordIdImport(650, 'CODE', values[colIndex], fileRowIndex, colIndex + colOffset, false)
    ...
    // графа 8
    colIndex++
    newRow.dateRegistration = parseDate(values[colIndex], "dd.MM.yyyy", fileRowIndex, colIndex + colOffset, logger, true)

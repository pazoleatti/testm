package form_template.vat.vat_937_3_3q2016.v2016
...
/**
 * (937.3) Итоговые данные из журнала полученных и выставленных счетов-фактур по посреднической деятельности (с 3 квартала 2016).
 * formTemplateId=859
 */
...
void logicCheck() {
    ...
    for (def row : dataRows) {
        ...
        // 4. Проверка значения «Графы 12» (часть 2)
    ...
    // 6. Проверка итоговых значений (графа 14..19)
...
def getNewRow(String[] rowCells, def columnCount, def fileRowIndex, def rowIndex) {
    ...
    // графа 3
    colIndex++
    newRow.opTypeCode = getRecordIdImport(650, 'CODE', pure(rowCells[colIndex]), fileRowIndex, colIndex + colOffset, false)

    // графа 4..13
    ['invoiceNumDate', 'invoiceCorrNumDate', 'corrInvoiceNumDate', 'corrInvCorrNumDate', 'buyerName',
...
def getNewRowFromXls(def values, def colOffset, def fileRowIndex, def rowIndex) {
    ...
    // графа 3
    colIndex = 3
    newRow.opTypeCode = getRecordIdImport(650, 'CODE', values[colIndex], fileRowIndex, colIndex + colOffset, false)

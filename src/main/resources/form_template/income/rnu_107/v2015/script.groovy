...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[1][4]): tmpRow.getCell('transDoneDate').column.name]),
            ([(headerRows[1][5]): tmpRow.getCell('code').column.name]),
            ([(headerRows[1][6]): 'Номер']),
            ([(headerRows[1][7]): 'Дата']),
            ([(headerRows[1][8]): tmpRow.getCell('sum1').column.name]),
            ([(headerRows[1][9]): tmpRow.getCell('dealTariff').column.name]),
            ([(headerRows[1][10]): tmpRow.getCell('taxTariff').column.name]),
            ([(headerRows[1][11]): tmpRow.getCell('sum2').column.name]),
            ([(headerRows[1][12]): tmpRow.getCell('sum3').column.name]),
            ([(headerRows[1][13]): tmpRow.getCell('sum4').column.name]),
    ]
    (2..13).each {

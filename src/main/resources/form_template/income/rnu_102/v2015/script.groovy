...
void logicCheck() {
    ...
    for (row in dataRows) {
        ...
        if (both) {
            // a
            logger.error("Строка $rowNum: Должна быть заполнена только одна из граф: «$taxPrice» или «$outcomeRate»!")
        } else if (noOne) {
            // b
            logger.error("Строка $rowNum: Должна быть заполнена одна из граф: «$taxPrice» или «$outcomeRate»!")
        }

        // 9. Проверка суммы фактически начисленного расхода
        ...
        } else if (row.sum1 != null && row.outcomeRate == null && row.sum1 < 0) {
            // b
            def msg13 = row.getCell('sum1').column.name
        ...
        if (row.sum2 != null) {
            ...
            } else if (row.outcomeRate == null && row.sum1 > 0 && row.taxPrice != null && row.count != null && row.course != null) {
                // b
                if (row.sum2 != calc15(row, false)) {
                    logger.error("Строка $rowNum: Значение графы «$msg15» должно быть равно произведению значений граф «$msg12», «$msg10» и «$msg6»!")
                }
            } else if (row.sum1 == 0 && row.taxPrice != null && row.outcomeRate == null) {
                // c
                if (row.sum2 != calc15(row, false)) {

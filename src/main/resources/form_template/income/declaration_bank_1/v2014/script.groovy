...
void generateXML() {
    ...
    def dataRowsDividend = getDataRows(formDataCollection, departmentId, newDeclaration ? 10141 : 306, FormDataKind.ADDITIONAL)

    /** Расчет налога на прибыль с доходов, удерживаемого налоговым агентом. */
    /** либо */
    /** Сведения о дивидендах, выплаченных в отчетном квартале. */
    def dataRowsTaxAgent = getDataRows(formDataCollection, departmentId,  newDeclaration ? 10144 : 307, FormDataKind.ADDITIONAL)

    /** Сумма налога, подлежащая уплате в бюджет, по данным налогоплательщика. */
    def dataRowsTaxSum = getDataRows(formDataCollection, departmentId,  newDeclaration ? 10142 : 308, FormDataKind.ADDITIONAL)

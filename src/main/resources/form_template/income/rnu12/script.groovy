    ...
    if (!formData.dataRows.isEmpty()) {
        ...
        for (def row : formData.dataRows) {
            ...
            if (row.periodCounts == 0) {
                logger.warn('Деление на ноль. При проверке значения поля «Сумма расхода, начисленная в налоговом учёте».')
            }
            tmp = round(row.advancePayment / row.periodCounts, 2)
            if (row.outcomeInNalog != tmp) {
                logger.warn('Неверное значение поля «Сумма расхода, начисленная в налоговом учёте»!')
            }

...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[1][7]): 'Дата']),
            ([(headerRows[1][8]): tmpRow.getCell('base').column.name]),
            ([(headerRows[1][9]): tmpRow.getCell('sum').column.name]),
            ([(headerRows[1][10]): tmpRow.getCell('currency').column.name]),
            ([(headerRows[1][11]): tmpRow.getCell('time').column.name]),
            ([(headerRows[1][12]): tmpRow.getCell('rate').column.name]),
            ([(headerRows[1][13]): tmpRow.getCell('sum1').column.name]),
            ([(headerRows[1][14]): tmpRow.getCell('rate1').column.name]),
            ([(headerRows[1][15]): tmpRow.getCell('sum2').column.name]),
            ([(headerRows[1][16]): tmpRow.getCell('rate2').column.name]),
            ([(headerRows[1][17]): tmpRow.getCell('sum3').column.name]),
            ([(headerRows[2][0]): '1'])
    ]
    (2..16).each {

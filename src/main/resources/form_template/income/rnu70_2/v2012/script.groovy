...
def getCalcTotalNameRow(def row, def dataRows) {
    ...
    def rowNumber = getRowNumber(tRow.name, dataRows)
    def newRow = getSubTotalRow(rowNumber, tRow.name)
...
void addTransportData(def xml) {
    ...
    for (def row : xml.row) {
        ...
        // графа 12..19
        def colIndex = 11
        ['rateBR', 'interestRate', 'perc', 'loss', 'marketPrice', 'financialResult2', 'maxLoss', 'financialResultAdjustment'].each { alias ->
            colIndex++
            newRow[alias] = parseNumber(row.cell[colIndex].text(), rnuIndexRow, colIndex + colOffset, logger, true)
        }
    ...
    if (!totalRowFromFileMap.isEmpty()) {
        ...
        totalRowsMap.values().toArray().each { tmpSubTotalRow ->
            ...
            if (totalRows) {
                ...
                rowWarning(logger, null, String.format(GROUP_WRONG_ITOG, tmpSubTotalRow.fix.replaceAll("Итого по КНУ ", "")))
            }
        }
        if (!totalRowFromFileMap.isEmpty()) {
...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[0][16]): getColumnName(tmpRow, 'marketPrice')]),
            ([(headerRows[0][17]): getColumnName(tmpRow, 'financialResult2')]),
            ([(headerRows[0][18]): getColumnName(tmpRow, 'maxLoss')]),
            ([(headerRows[0][19]): getColumnName(tmpRow, 'financialResultAdjustment')]),
...
def getNewSubTotalRowFromXls(def values, def colOffset, def fileRowIndex, def rowIndex) {
    ...
    // графа 19
    colIndex = 19
    newRow.financialResultAdjustment = parseNumber(values[colIndex], fileRowIndex, colIndex + colOffset, logger, true)

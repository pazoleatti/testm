...
void addTransportData(def xml) {
    ...
    for (def row : xml.row) {
        ...
        // графа 16
        newRow.marketPrice = parseNumber(row.cell[16].text(), rnuIndexRow, 16 + colOffset, logger, true)
        // графа 17
        newRow.financialResult2 = parseNumber(row.cell[17].text(), rnuIndexRow, 17 + colOffset, logger, true)
        // графа 18
        newRow.maxLoss = parseNumber(row.cell[18].text(), rnuIndexRow, 18 + colOffset, logger, true)
        // графа 19
        newRow.financialResultAdjustment = parseNumber(row.cell[19].text(), rnuIndexRow, 19 + colOffset, logger, true)

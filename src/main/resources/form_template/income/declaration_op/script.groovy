...
if (bankDeclarationData == null || !bankDeclarationData.accepted) {
    logger.error('Декларация Банка по прибыли не сформирована или не принята.')
    return
}
...
if (!findCurrentDepo) {
    logger.error('В декларации Банка отсутствуют данные для данного подразделения (в приложении № 5 к Листу 02).')
    return
}

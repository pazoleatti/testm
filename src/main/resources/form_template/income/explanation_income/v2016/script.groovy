...
void logicCheck() {
    ...
    for (def row : dataRows) {
        ...
        if (row.sum != null && !(row.sum > BigDecimal.ZERO)) {
            logger.error("Строка ${row.getIndex()}: Значение графы «${getColumnName(rows[0], 'sum')}» должно быть больше «0»!")
        }
    }
...
void checkHeaderXls(def headerRows, def colCount, rowCount, def tmpRow) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[0][2]): getColumnName(tmpRow, 'symbol')]),
            ([(headerRows[0][3]): getColumnName(tmpRow, 'sum')]),
            ([(headerRows[0][4]): getColumnName(tmpRow, 'explanation')]),
    ]
    (1..5).each { index ->

...
def getNewRow(String[] rowCells, def columnCount, def fileRowIndex, def rowIndex, def isTotal = false) {
    ...
    // графа 7
    colIndex++
    newRow.currencyCode = getRecordIdImport(15, 'CODE', pure(rowCells[colIndex]), fileRowIndex, colIndex + colOffset, false)
...
boolean compareGroup(def rowA, def rowB) {
    String valueA = (rowA.getAlias() != null) ? rowA.helper : getTitle(getKnu(rowA.code))
    String valueB = (rowB.getAlias() != null) ? rowB.helper : getTitle(getKnu(rowB.code))
    def var = !valueA.equals(valueB)
    return var
}

package form_template.income.rnu6.v2012
...
void addData(def xml, int headRowCount) {
    ...
    for (def row : xml.row) {
        ...
        newRow.code = getRecordIdImport(28, 'CODE', row.cell[2].text(), xlsIndexRow, 2 + colOffset, false)
        def map = getRefBookValue(28, newRow.code)

        // графа 4 проверка
        if (map != null) {
            formDataService.checkReferenceValue(28, row.cell[4].text(), map.NUMBER?.stringValue, xlsIndexRow, 4 + colOffset, logger, false)
        }
        ...
        // графа 7
        newRow.currencyCode = getRecordIdImport(15, 'CODE', row.cell[7].text(), xlsIndexRow, 7 + colOffset, false)

...
void logicCheck() {
    ...
    if (stoimRealPTDoSr != null && viruchRealPTDoSr != null && ubit1Prev269 != null && ubit1Soot269 != null &&
            ...
            : (ubit1Prev269 != 0))) {
        logger.warn('В Приложении № 3 к Листу 02 не пройдена проверка отрицательной разницы (убытка) от реализации права требования долга до наступления срока платежа, определенной налогоплательщиком в соответствии с п. 1 статьи 279 НК.')
    }
    ...
    if (stoimRealPTPosSr != null && viruchRealPTPosSr != null && ubit2RealPT != null &&
            ...
            : (ubit2RealPT != 0))) {
        logger.warn('В Приложении № 3 к Листу 02 не пройдена проверка отрицательной разницы (убытка), полученной налогоплательщиком при уступке права требования долга после наступления срока платежа в соответствии с п. 2 статьи 279 НК.')
    }
}
...
void generateXML() {
    ...
    def dataRowsDividend = getDataRows(formDataCollection, 306, FormDataKind.ADDITIONAL)

    /** Расчет налога на прибыль с доходов, удерживаемого налоговым агентом. */
    def dataRowsTaxAgent = getDataRows(formDataCollection, 307, FormDataKind.ADDITIONAL)

    /** Выходная налоговая форма «Сумма налога, подлежащая уплате в бюджет, по данным налогоплательщика». */
    def dataRowsTaxSum = getDataRows(formDataCollection, 308, FormDataKind.ADDITIONAL)

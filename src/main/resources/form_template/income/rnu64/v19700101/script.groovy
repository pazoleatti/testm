package form_template.income.rnu64.v19700101
...
def logicalCheck() {
        ...
        if (!isTotalRow(row)) {
            ...
            getRows(data).each { rowItem ->
                if (!isTotalRow(row) && row.number == rowItem.number && !row.equals(rowItem)) {
                    loggerError("В строке " + row.number + " нарушена уникальность номера по порядку!")//TODO вернуть error
                    return false
                }
            }
            ...
            if (row.costs == 0) {
                loggerError("В строке " + row.number + " все суммы по операции нулевые!")//TODO вернуть error
                return false
            }
    ...
    if (totalQuarterRow != null || totalRow != null) {
        ...
        if (totalQuarterRow != null && totalQuarterRow.costs != getQuarterTotal()) {
            loggerError('Итоговые значения за текущий квартал рассчитаны неверно!')//TODO вернуть error
            return false
        }
        ...
        if (totalRow != null && totalRow.costs != getTotalValue()) {
            loggerError('Итоговые значения за текущий отчётный (налоговый ) период рассчитаны неверно!')//TODO вернуть error
            return false
        }
    }
    return true

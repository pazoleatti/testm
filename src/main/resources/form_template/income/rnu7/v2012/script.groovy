...
void addData(def xml, int headRowCount) {
    ...
    for (def row : xml.row) {
        ...
        // графа 4
        String filter = "LOWER(CODE) = LOWER('" + row.cell[2].text() + "') and LOWER(NUMBER) = LOWER('" + row.cell[xmlIndexCol].text().replaceAll(/\./, "") + "')"
        def records = refBookFactory.getDataProvider(27).getRecords(reportPeriodEndDate, null, filter, null)
        if (checkImportRecordsCount(records, refBookFactory.get(27), 'CODE', row.cell[2].text(), reportPeriodEndDate, xlsIndexRow, 2, logger, false)) {
        ...
        // графа 7
        newRow.currencyCode = getRecordIdImport(15, 'CODE', row.cell[xmlIndexCol].text(), xlsIndexRow, xmlIndexCol + colOffset, false)
        xmlIndexCol++

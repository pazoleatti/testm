package form_template.income.rnu31.v19700101
...
def logicalCheck() {
    ...
    // Ошибка при поиске формы (убираю из 0.3.3)
...
void consolidation() {
    ...
    /*departmentFormTypeService.getFormSources(formDataDepartment.id, formData.getFormType().getId(), formData.getKind()).each {
        if (it.formTypeId == formData.getFormType().getId()) {
            ...
            if (source != null && source.state == WorkflowState.ACCEPTED) {
                ...
                columns.each { alias ->
                   sums[alias] += sourceRow.getCell(alias).getValue()
                }
            }
        }
    }*/
    ...
    columns.each { alias ->
        row.getCell(alias).setValue(sums[alias])
    }

...
// графа 3  - comparePeriod			Период сравнения. НДС всего, млн. руб.
// графа 4  - comparePeriodIgnore	Период сравнения. В том числе НДС не учитываемый, млн. руб.
// графа 5  - comparePeriodPercent	Период сравнения. Доля НДС не учитываемый, %
// графа 6  - currentPeriod			Период. НДС всего, млн. руб.
// графа 7  - currentPeriodIgnore	Период. В том числе НДС не учитываемый, млн. руб.
// графа 8  - currentPeriodPercent	Период. Доля НДС не учитываемый, %
// графа 9  - delta					Изменения за период. НДС всего, млн. руб.
// графа 10 - deltaIgnore			Изменения за период. В том числе НДС не учитываемый, млн. руб.
        ...
        if (it.formTypeId == sourceFormTypeId) {
            ...
            if (source != null && source.state == WorkflowState.ACCEPTED) {
                ...
                commonColumns.each { column ->
                    newRow[column] = row[column] / 1000 // тысячи переводим в миллионы
                }
                dataRows.add(newRow)
            }
        }
...
void checkHeaderXls(def headerRows, def colCount, rowCount) {
    ...
    def headerMapping = [
            ...
            ([(headerRows[1][2]) : 'НДС всего, млн. руб.']),
            ([(headerRows[1][3]) : 'В том числе НДС не учитываемый, млн. руб.']),
            ([(headerRows[1][4]) : 'Доля НДС не учитываемый, %']),
            ([(headerRows[1][5]) : 'НДС всего, млн. руб.']),
            ([(headerRows[1][6]) : 'В том числе НДС не учитываемый, млн. руб.']),
            ([(headerRows[1][7]) : 'Доля НДС не учитываемый, %']),
            ([(headerRows[1][8]) : 'НДС всего, млн. руб.']),
            ([(headerRows[1][9]) : 'В том числе НДС не учитываемый, млн. руб.']),

package form_template.etr.etr_4_5.v2015
...
/**
 * Приложение 4-5. Отношение налогов, относимых на расходы к операционным расходам.
 * formTemplateId = 705
 *
...
def opuMap = [ 'R1' : ['26411.01', '26411.02', '26411.03', '26102', '26410.09', '26411.12', '26411.13'] ]
...
def getSourceValue(def periodId, def row, def alias, def isCalc) {
    ...
    if (periodId != null) {
        ...
        periods.each { period ->
            ...
            if (sourceForm != null) {
                sum += (sourceForm?.allSaved?.get(3)?.sum ?: 0)
            } else {
                notFound404 = true
                if (isCalc) { // выводить только при расчете
                    // 4. Проверка наличия принятой источника «Величины налоговых платежей, вводимые вручную» (предрасчетные проверки)

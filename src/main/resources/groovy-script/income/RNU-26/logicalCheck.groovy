...
// графа 17 - reserveRecovery
        ...
        if (formDataOld != null) {
            ...
            // 12.		Проверка корректности заполнения РНУ	Если «графа  4» = «графа 4» формы «Регистр налогового учёта расчёта резерва под возможное обесценение акций, РДР, ADR, GDR и опционов эмитента в целях налогообложения» за предыдущий отчётный период, то «графа 8»  = «графа 15» формы «Регистр налогового учёта расчёта резерва под возможное обесценение акций, РДР, ADR, GDR и опционов эмитента в целях налогообложения» за предыдущий отчётный период	1	РНУ сформирован некорректно! Не выполняется условие: <текст проверки>

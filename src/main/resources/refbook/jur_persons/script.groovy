...
void save() {
    ...
    saveRecords.each {
        ...
        // 6. Проверка правильности заполнения полей «Дата включения в ВЗЛ» и «Дата исключения из ВЗЛ»
        if (type == "ВЗЛ" && startDate != null && endDate != null && startDate > endDate) {
            logger.error("Поле «%s» должно быть меньше или равно полю «%s»!", getAttrName('START_DATE'), getAttrName('END_DATE'))
        }

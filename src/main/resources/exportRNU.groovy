...
enum Exclude {
    ...
    TOTAL2("itog"), // исключать строки содержащие в алиасе itog
...
// 3. исключать строки если в их алиасе есть заданное значение (например «total») - Exclude.SECTION_TOTAL
...
def formTemplatesExcludeRowMap = [
        ...
        1372 : Exclude.FIX,           // (Приложение 5) Сведения для расчета налога на прибыль
        ...
        10618: Exclude.SECTION_TOTAL, // Сводный регистр налогового учета по формированию и использованию резерва по сомнительным долгам.
        ...
        336  : Exclude.SECTION2_TOTAL, // (РНУ-39.1) Регистр налогового учёта процентного дохода по коротким позициям. Отчёт 1
        337  : Exclude.SECTION2_TOTAL, // (РНУ-39.2) Регистр налогового учёта процентного дохода по коротким позициям
        ...
        // НДС
        600  : Exclude.SECTION_TOTAL,  // (724.1) Отчёт о суммах начисленного НДС по операциям Банка
        1600 : Exclude.SECTION_TOTAL,  // (724.1) Отчёт о суммах начисленного НДС по операциям Банка (2015)
        601  : Exclude.TOTAL2,         // (724.2.1) Операции, не подлежащие налогообложению (освобождаемые от налогообложения), операции, не признаваемые объектом налогообложения, операции по реализации товаров (работ, услуг), местом реализации которых не признается территория Российской Федерации, а также суммы оплаты, частичной оплаты в счет предстоящих поставок (выполнения работ, оказания услуг), длительность производственного цикла изготовления которых составляет свыше шести месяцев
        10601: Exclude.TOTAL2,         // (724.2.1) Операции, не подлежащие налогообложению (освобождаемые от налогообложения), не признаваемые объектом налогообложения, операции по реализации товаров (работ, услуг), местом реализации которых не признается территория Российской Федерации
        602  : Exclude.TOTAL2,         // (724.2.2) Расчёт суммы налога по операциям по реализации товаров (работ, услуг), обоснованность применения налоговой ставки 0 процентов по которым документально подтверждена
        1602 : Exclude.TOTAL2,         // (724.2.2) Расчёт суммы налога по операциям по реализации товаров (работ, услуг), обоснованность применения налоговой ставки 0 процентов по которым документально подтверждена (2015)
        603  : Exclude.SECTION_TOTAL,  // (724.4) Отчёт по сумме НДС, уплаченного в бюджет, в случае возврата ранее реализованных товаров (отказа от услуг) или возврата соответствующих сумм авансовых платежей
        1603 : Exclude.SECTION_TOTAL,  // (724.4) Отчёт по сумме НДС, уплаченного в бюджет, в случае возврата ранее реализованных товаров (отказа от услуг) или возврата соответствующих сумм авансовых платежей
        604  : Exclude.FIX,            // (724.6) Отчёт о суммах НДС начисленных налоговым агентом с сумм дохода иностранных юридических лиц (балансовый счёт 60309.02)
        605  : Exclude.FIX,            // (724.7) Отчёт о суммах НДС начисленных налоговым агентом по договорам аренды имущества (балансовый счёт 60309.03)
        606  : Exclude.NO,             // (937.1) Итоговые данные книги покупок
        1606 : Exclude.FIX,            // (937.1) Итоговые данные книги покупок (2015)
        607  : Exclude.TOTAL,          // (937.1.13) Расшифровка графы 13 "Расхождение" формы 937.1
        608  : Exclude.NO,             // (937.2) Итоговые данные книги продаж
        1608 : Exclude.FIX,            // (937.2) Итоговые данные книги продаж (2015)
        609  : Exclude.TOTAL,          // (937.2.13) Расшифровка графы 13 "Расхождение" формы 937.2
        616  : Exclude.FIX,            // (937.1.1) Сведения из дополнительных листов книги покупок
        617  : Exclude.FIX,            // (937.2.1) Сведения из дополнительных листов книги продаж
        619  : Exclude.SECTION_TOTAL,  // (937.3) Итоговые данные из журнала полученных и выставленных счетов-фактур по посреднической деятельности
        // имущество
        610 : Exclude.NO
]
...
def sectionsMap = [
        ...
        10618 : ['А', 'Б'],    // Сводный регистр налогового учета по формированию и использованию резерва по сомнительным долгам.
        ...
        204   : ['A', 'B', 'C']    // Сведения о транспортных средствах, по которым уплачивается транспортный налог (new)
]
...
def createFiles() {
    ...
    dataRowsMap.each { id, dataRows ->
        ...
        def index = 0
        def sectionIndex = 0
        // для макета 329 (рну 30) и 10618 (сомнительные долги) первые строки это раздел 0
        def sectionValue = (id == 329 || id == 10618 ? '0' : null)
...
def isExcludeRow(def row, Exclude exclude) {
    if (row.getAlias() != null) {
        if (exclude == Exclude.FIX || exclude == Exclude.SECTION_TOTAL || exclude == Exclude.SECTION2_TOTAL) {
            return true
        } else if ((exclude == Exclude.TOTAL && row.getAlias().contains(Exclude.TOTAL.value)) || (exclude == Exclude.TOTAL2 && row.getAlias().contains(Exclude.TOTAL2.value))) {
            return true

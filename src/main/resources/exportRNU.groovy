...
def formTemplatesExcludeRowMap = [
        ...
        10980: Exclude.SECTION_TOTAL, // Сводный регистр налогового учета по формированию и использованию резерва по сомнительным долгам.
        ...
        1600: Exclude.SECTION_TOTAL, // Отчёт о суммах начисленного НДС по операциям Банка (2015)
        601 : Exclude.TOTAL2, // Операции, не подлежащие налогообложению (освобождаемые от налогообложения), операции, не признаваемые объектом налогообложения, операции по реализации товаров (работ, услуг), местом реализации которых не признается территория Российской Федерации, а также суммы оплаты, частичной оплаты в счет предстоящих поставок (выполнения работ, оказания услуг), длительность производственного цикла изготовления которых составляет свыше шести месяцев
        602 : Exclude.TOTAL2, // Расчёт суммы налога по операциям по реализации товаров (работ, услуг), обоснованность применения налоговой ставки 0 процентов по которым документально подтверждена
        603 : Exclude.SECTION_TOTAL, // Отчёт по сумме НДС, уплаченного в бюджет, в случае возврата ранее реализованных товаров (отказа от услуг) или возврата соответствующих сумм авансовых платежей
        604 : Exclude.FIX, // Отчёт о суммах НДС начисленных налоговым агентом с сумм дохода иностранных юридических лиц (балансовый счёт 60309.02)
        605 : Exclude.FIX, // Отчёт о суммах НДС начисленных налоговым агентом по договорам аренды имущества (балансовый счёт 60309.03)
        606 : Exclude.NO, // Итоговые данные книги покупок
        1606: Exclude.FIX, // Итоговые данные книги покупок (2015)
        607 : Exclude.TOTAL, // Расшифровка графы 13 "Расхождение" формы 937.1
        608 : Exclude.NO, // Итоговые данные книги продаж
        1608: Exclude.FIX, // Итоговые данные книги продаж (2015)
        609 : Exclude.TOTAL, // Расшифровка графы 13 "Расхождение" формы 937.2
        616 : Exclude.FIX, // Сведения из дополнительных листов книги покупок
        617 : Exclude.FIX, // Сведения из дополнительных листов книги продаж
        // имущество
        610 : Exclude.NO
]
...
def sectionsMap = [
        ...
        10980 : ['А', 'Б'],    // Сводный регистр налогового учета по формированию и использованию резерва по сомнительным долгам.
...
def createFiles() {
    ...
    dataRowsMap.each { id, dataRows ->
        ...
        def index = 0
        def sectionIndex = 0
        // для макета 329 (рну 30) и 10980 (сомнительные долги) первые строки это раздел 0
        def sectionValue = (id == 329 || id == 10980 ? '0' : null)

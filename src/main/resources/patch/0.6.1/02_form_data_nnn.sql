...
begin
for x in (select id from form_column where alias = 'status' and form_template_id = 10080 and type='S') loop
...
-- D->S: Номер мемориального ордера
begin
for x in (select fc.*,translate( alias || ' - ' || name, '''', ' ') as col_comment from form_column fc where form_template_id = 605 and alias = 'orderNumber' and type='D') loop
    execute immediate 'LOCK TABLE FORM_DATA_605 IN EXCLUSIVE MODE';
    execute immediate 'ALTER TABLE FORM_DATA_605 RENAME COLUMN C'||x.id||' TO C'||x.id||'_TEMP';
    execute immediate 'ALTER TABLE FORM_DATA_605 ADD C'||x.id||' varchar2(4000)';
	execute immediate 'COMMENT ON COLUMN FORM_DATA_605.C'||x.id||' IS '''||x.col_comment||'''';
    execute immediate 'UPDATE FORM_DATA_605 SET C'||x.id||'= to_char(C'||x.id||'_TEMP , ''DD.MM.YYYY'')';
    execute immediate 'ALTER TABLE FORM_DATA_605 DROP COLUMN C'||x.id||'_TEMP';
end loop;
end;
/

--update form_column set type='S', max_length=256, format=null where form_template_id = 605 and alias = 'orderNumber';

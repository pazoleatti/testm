...
select seq_department_form_type.nextval, 1, 10540, 5 from dual where exists(select 1 from ref_book_attribute where id in (205, 206));
...
select seq_form_data.nextval, 10800, 4, 5, drp.id, 0  
from tax_period tp
...
where form_template_id = 10800 and where exists(select 1 from ref_book_attribute where id in (205, 206));

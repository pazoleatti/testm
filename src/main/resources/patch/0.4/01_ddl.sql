...
INSERT INTO configuration_email (id, name, value, description) values (2, 'mail.smtp.password', '', 'The password');
INSERT INTO configuration_email (id, name, value, description) values (3, 'mail.smtp.host', '', 'The SMTP server to connect to.');
INSERT INTO configuration_email (id, name, value, description) values (4, 'mail.smtp.port', '', 'The SMTP server port to connect to, if the connect() method doesn''t explicitly specify one. Defaults to 25.');
INSERT INTO configuration_email (id, name, value, description) values (5, 'mail.smtp.connectiontimeout', '', 'Socket connection timeout value in milliseconds. This timeout is implemented by java.net.Socket. Default is infinite timeout.');
INSERT INTO configuration_email (id, name, value, description) values (6, 'mail.smtp.timeout', '', 'Socket read timeout value in milliseconds. This timeout is implemented by java.net.Socket. Default is infinite timeout.');
INSERT INTO configuration_email (id, name, value, description) values (7, 'mail.smtp.writetimeout', '', 'Socket write timeout value in milliseconds. This timeout is implemented by using a java.util.concurrent.ScheduledExecutorService per connection that schedules a thread to close the socket if the timeout expires. Thus, the overhead of using this timeout is one thread per connection. Default is infinite timeout.');
INSERT INTO configuration_email (id, name, value, description) values (8, 'mail.smtp.from', '', 'Email address to use for SMTP MAIL command. This sets the envelope return address. Defaults to msg.getFrom() or InternetAddress.getLocalAddress(). NOTE: mail.smtp.user was previously used for this.');
INSERT INTO configuration_email (id, name, value, description) values (9, 'mail.smtp.localhost', '', 'Local host name used in the SMTP HELO or EHLO command. Defaults to InetAddress.getLocalHost().getHostName(). Should not normally need to be set if your JDK and your name service are configured properly.');
INSERT INTO configuration_email (id, name, value, description) values (10, 'mail.smtp.localaddress', '', 'Local address (host name) to bind to when creating the SMTP socket. Defaults to the address picked by the Socket class. Should not normally need to be set, but useful with multi-homed hosts where it''s important to pick a particular local address to bind to.');
INSERT INTO configuration_email (id, name, value, description) values (11, 'mail.smtp.localport', '', 'Local port number to bind to when creating the SMTP socket. Defaults to the port number picked by the Socket class.');
INSERT INTO configuration_email (id, name, value, description) values (12, 'mail.smtp.ehlo', '', 'If false, do not attempt to sign on with the EHLO command. Defaults to true. Normally failure of the EHLO command will fallback to the HELO command; this property exists only for servers that don''t fail EHLO properly or don''t implement EHLO properly.');
INSERT INTO configuration_email (id, name, value, description) values (13, 'mail.smtp.auth', '', 'If true, attempt to authenticate the user using the AUTH command. Defaults to false.');
INSERT INTO configuration_email (id, name, value, description) values (14, 'mail.smtp.auth.mechanisms', '', 'If set, lists the authentication mechanisms to consider, and the order in which to consider them. Only mechanisms supported by the server and supported by the current implementation will be used. The default is "LOGIN PLAIN DIGEST-MD5 NTLM", which includes all the authentication mechanisms supported by the current implementation.');
INSERT INTO configuration_email (id, name, value, description) values (15, 'mail.smtp.auth.login.disable', '', 'If true, prevents use of the AUTH LOGIN command. Default is false.');
INSERT INTO configuration_email (id, name, value, description) values (16, 'mail.smtp.auth.plain.disable', '', 'If true, prevents use of the AUTH PLAIN command. Default is false.');
INSERT INTO configuration_email (id, name, value, description) values (17, 'mail.smtp.auth.digest-md5.disable', '', 'If true, prevents use of the AUTH DIGEST-MD5 command. Default is false.');
INSERT INTO configuration_email (id, name, value, description) values (18, 'mail.smtp.auth.ntlm.disable', '', 'If true, prevents use of the AUTH NTLM command. Default is false.');
INSERT INTO configuration_email (id, name, value, description) values (19, 'mail.smtp.auth.ntlm.domain', '', 'The NTLM authentication domain.');
INSERT INTO configuration_email (id, name, value, description) values (20, 'mail.smtp.auth.ntlm.flags', '', 'NTLM protocol-specific flags. See http://curl.haxx.se/rfc/ntlm.html#theNtlmFlags for details.');
INSERT INTO configuration_email (id, name, value, description) values (21, 'mail.smtp.submitter', '', 'The submitter to use in the AUTH tag in the MAIL FROM command. Typically used by a mail relay to pass along information about the original submitter of the message. See also the setSubmitter method of SMTPMessage. Mail clients typically do not use this.');
INSERT INTO configuration_email (id, name, value, description) values (22, 'mail.smtp.dsn.notify', '', 'The NOTIFY option to the RCPT command. Either NEVER, or some combination of SUCCESS, FAILURE, and DELAY (separated by commas).');
INSERT INTO configuration_email (id, name, value, description) values (23, 'mail.smtp.dsn.ret', '', 'The RET option to the MAIL command. Either FULL or HDRS.');
INSERT INTO configuration_email (id, name, value, description) values (24, 'mail.smtp.allow8bitmime', '', 'If set to true, and the server supports the 8BITMIME extension, text parts of messages that use the "quoted-printable" or "base64" encodings are converted to use "8bit" encoding if they follow the RFC2045 rules for 8bit text.');
...
INSERT INTO configuration_email (id, name, value, description) values (26, 'mail.smtp.sasl.enable', '', 'If set to true, attempt to use the javax.security.sasl package to choose an authentication mechanism for login. Defaults to false.');
INSERT INTO configuration_email (id, name, value, description) values (27, 'mail.smtp.sasl.mechanisms', '', 'A space or comma separated list of SASL mechanism names to try to use.');
INSERT INTO configuration_email (id, name, value, description) values (28, 'mail.smtp.sasl.authorizationid', '', 'The authorization ID to use in the SASL authentication. If not set, the authentication ID (user name) is used.');
INSERT INTO configuration_email (id, name, value, description) values (29, 'mail.smtp.sasl.realm', '', 'The realm to use with DIGEST-MD5 authentication.');
INSERT INTO configuration_email (id, name, value, description) values (30, 'mail.smtp.sasl.usecanonicalhostname', '', 'If set to true, the canonical host name returned by InetAddress.getCanonicalHostName is passed to the SASL mechanism, instead of the host name used to connect. Defaults to false.');
INSERT INTO configuration_email (id, name, value, description) values (31, 'mail.smtp.quitwait', '', 'If set to false, the QUIT command is sent and the connection is immediately closed. If set to true (the default), causes the transport to wait for the response to the QUIT command.');
INSERT INTO configuration_email (id, name, value, description) values (32, 'mail.smtp.reportsuccess', '', 'If set to true, causes the transport to include an SMTPAddressSucceededException for each address that is successful. Note also that this will cause a SendFailedException to be thrown from the sendMessage method of SMTPTransport even if all addresses were correct and the message was sent successfully.');
INSERT INTO configuration_email (id, name, value, description) values (33, 'mail.smtp.socketFactory', '', 'If set to a class that implements the javax.net.SocketFactory interface, this class will be used to create SMTP sockets. Note that this is an instance of a class, not a name, and must be set using the put method, not the setProperty method.');
INSERT INTO configuration_email (id, name, value, description) values (34, 'mail.smtp.socketFactory.class', '', 'If set, specifies the name of a class that implements the javax.net.SocketFactory interface. This class will be used to create SMTP sockets.');
INSERT INTO configuration_email (id, name, value, description) values (35, 'mail.smtp.socketFactory.fallback', '', 'If set to true, failure to create a socket using the specified socket factory class will cause the socket to be created using the java.net.Socket class. Defaults to true.');
INSERT INTO configuration_email (id, name, value, description) values (36, 'mail.smtp.socketFactory.port', '', 'Specifies the port to connect to when using the specified socket factory. If not set, the default port will be used.');
INSERT INTO configuration_email (id, name, value, description) values (37, 'mail.smtp.ssl.enable', '', 'If set to true, use SSL to connect and use the SSL port by default. Defaults to false for the "smtp" protocol and true for the "smtps" protocol.');
INSERT INTO configuration_email (id, name, value, description) values (38, 'mail.smtp.ssl.checkserveridentity', '', 'If set to true, check the server identity as specified by RFC 2595. These additional checks based on the content of the server''s certificate are intended to prevent man-in-the-middle attacks. Defaults to false.');
INSERT INTO configuration_email (id, name, value, description) values (39, 'mail.smtp.ssl.trust', '', 'If set, and a socket factory hasn''t been specified, enables use of a MailSSLSocketFactory. If set to "*", all hosts are trusted. If set to a whitespace separated list of hosts, those hosts are trusted. Otherwise, trust depends on the certificate the server presents.');
INSERT INTO configuration_email (id, name, value, description) values (40, 'mail.smtp.ssl.socketFactory', '', 'If set to a class that extends the javax.net.ssl.SSLSocketFactory class, this class will be used to create SMTP SSL sockets. Note that this is an instance of a class, not a name, and must be set using the put method, not the setProperty method.');
INSERT INTO configuration_email (id, name, value, description) values (41, 'mail.smtp.ssl.socketFactory.class', '', 'If set, specifies the name of a class that extends the javax.net.ssl.SSLSocketFactory class. This class will be used to create SMTP SSL sockets.');
INSERT INTO configuration_email (id, name, value, description) values (42, 'mail.smtp.ssl.socketFactory.port', '', 'Specifies the port to connect to when using the specified socket factory. If not set, the default port will be used.');
INSERT INTO configuration_email (id, name, value, description) values (43, 'mail.smtp.ssl.protocols', '', 'Specifies the SSL protocols that will be enabled for SSL connections. The property value is a whitespace separated list of tokens acceptable to the javax.net.ssl.SSLSocket.setEnabledProtocols method.');
INSERT INTO configuration_email (id, name, value, description) values (44, 'mail.smtp.ssl.ciphersuites', '', 'Specifies the SSL cipher suites that will be enabled for SSL connections. The property value is a whitespace separated list of tokens acceptable to the javax.net.ssl.SSLSocket.setEnabledCipherSuites method.');
INSERT INTO configuration_email (id, name, value, description) values (45, 'mail.smtp.starttls.enable', '', 'If true, enables the use of the STARTTLS command (if supported by the server) to switch the connection to a TLS-protected connection before issuing any login commands. Note that an appropriate trust store must configured so that the client will trust the server''s certificate. Defaults to false.');
INSERT INTO configuration_email (id, name, value, description) values (46, 'mail.smtp.starttls.required', '', 'If true, requires the use of the STARTTLS command. If the server doesn''t support the STARTTLS command, or the command fails, the connect method will fail. Defaults to false.');
INSERT INTO configuration_email (id, name, value, description) values (47, 'mail.smtp.socks.host', '', 'Specifies the host name of a SOCKS5 proxy server that will be used for connections to the mail server. (Note that this only works on JDK 1.5 or newer.)');
INSERT INTO configuration_email (id, name, value, description) values (48, 'mail.smtp.socks.port', '', 'Specifies the port number for the SOCKS5 proxy server. This should only need to be used if the proxy server is not using the standard port number of 1080.');
INSERT INTO configuration_email (id, name, value, description) values (49, 'mail.smtp.mailextension', '', 'Extension string to append to the MAIL command. The extension string can be used to specify standard SMTP service extensions as well as vendor-specific extensions. Typically the application should use the SMTPTransport method supportsExtension to verify that the server supports the desired service extension. See RFC 1869 and other RFCs that define specific extensions.');
INSERT INTO configuration_email (id, name, value, description) values (50, 'mail.smtp.userset', '', 'If set to true, use the RSET command instead of the NOOP command in the isConnected method. In some cases sendmail will respond slowly after many NOOP commands; use of RSET avoids this sendmail issue. Defaults to false.');
INSERT INTO configuration_email (id, name, value, description) values (51, 'mail.smtp.noop.strict', '', 'If set to true (the default), insist on a 250 response code from the NOOP command to indicate success. The NOOP command is used by the isConnected method to determine if the connection is still alive. Some older servers return the wrong response code on success, some servers don''t implement the NOOP command at all and so always return a failure code. Set this property to false to handle servers that are broken in this way. Normally, when a server times out a connection, it will send a 421 response code, which the client will see as the response to the next command it issues. Some servers send the wrong failure response code when timing out a connection. Do not set this property to false when dealing with servers that are broken in this way.');
INSERT INTO configuration_email (id, name, value, description) values (52, 'mail.transport.protocol', '', 'Specifies the default message transport protocol. The Session method getTransport() returns a Transport object that implements this protocol. By default the first Transport provider in the configuration files is returned.');
---------------------------------------------------------------------------------------------------
COMMIT;
EXIT;
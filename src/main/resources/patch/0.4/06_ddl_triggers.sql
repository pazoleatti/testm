...
before delete on department_report_period
for each row
declare
...
begin
    ...
    if deleting then
       ...
       -- Запрет изменений, если на запись существуют ссылки
         if vHasLinks = 1 then
            raise_application_error(-20003, 'Нельзя изменить период, если на него есть ссылка в FORM_DATA или DECLARATION_DATA');
         end if; 
       end;
    end if;

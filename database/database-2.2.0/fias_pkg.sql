...
begin
  ...
  from (
        select t4.*,
          ...
          from (
                select t3.*,
                       fias_pkg.CheckAddrElementRetID(t3.region_code,t3.loc_fname,t3.loc_type,coalesce(t3.city_id,t3.area_id,t3.region_id),'LOCALITY',-1/*t3.loc_leaf*/) loc_id
                  from (
                        select t2.*,
                               case when t2.region_code in ('77','78','92','99') and t2.city is null then fias_pkg.CheckAddrElementRetID(t2.region_code,t2.city_fname,t2.city_type,null,'REGION',0)
                                    else fias_pkg.CheckAddrElementRetID(t2.region_code,t2.city_fname,t2.city_type,nvl(t2.area_id,t2.region_id),'CITY',t2.city_leaf) end city_id
                          from (
                                select t1.*,
                                  ...
                                  from (
                                      select t0.*, fias_pkg.CheckAddrElementRetID(t0.region_code,t0.area_fname,t0.area_type,null,'REGION',0)  region_id
                                      from
                                      ( 
                                        select n.id,
                                               n.post_index,n.region_code,n.area,n.city,n.locality,n.street,
                                          ...
                                          from ndfl_person n
                                         where n.declaration_data_id=p_declaration) t0
                                    ) t1
                               ) t2
                       ) t3
               ) t4

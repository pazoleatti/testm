...
select d.dep_id, seq_ref_book_record.nextval,n.id ndfl_id,seq_ref_book_record_row_id.nextval,trunc(sysdate,'y'),0,row_number() over (partition by t.depcode order by t.rowid) rn,
       t.tax_end,t.kpp,pp.id place_id,t.titname,
          ...
          from tmp_dep_params t join tmp_depart d on (d.code=t.depcode)
                                         ...
                                         left join ref_book_signatory_mark s on (s.code=t.sign and trunc(sysdate,'y') between s.version and trunc(sysdate,'y'))

create or replace package body person_pkg as
  ...
  */
  ...
  begin
    ...
    select max(end_date) version, -- Актуальная версия
           record_id,
           p_date calc_date
    from
      (
      -- Версии у которых указаны и Дата начала действия и Дата окончания действия
      select record_id, end_date 
      from ref_book_person 
      where start_date is not null and -- Дата начала действия
            start_date <= p_date and 
            end_date is not null and -- Дата окончания действия
            end_date >= p_date and 
            record_id=old_id -- не относится к дубликатам
      union
      -- Версии у которых указана Дата начала действия и не указана Дата окончания действия (т.е. Действие версии неограниченно во времени)
      select record_id, v_max_end_date -- Максимально возможная дата окончания действия 
      from ref_book_person 
      where start_date is not null and -- Дата начала действия
            start_date <= p_date and 
            end_date is null and -- Дата окончания действия
            record_id=old_id -- не относится к дубликатам
      union
      -- Версии у которых не указана Дата начала действия и указана Дата окончания действия
      select record_id, end_date 
      from ref_book_person 
      where start_date is null and -- Дата начала действия
            end_date is not null and -- Дата окончания действия
            end_date >= p_date and
            record_id=old_id -- не относится к дубликатам
      union
      -- Версии у которых не указаны Дата начала действия и Дата окончания действия
      select record_id, v_max_end_date -- Максимально возможная дата окончания действия 
      from ref_book_person 
      where start_date is null and -- Дата начала действия
            end_date is null and -- Дата окончания действия
        ...
        select fv.person_id,
          ...
          from (
             select /*+ use_hash(n fv)*/ distinct n.id as person_id,first_value(fv.id) over(partition by n.id order by fv.id) ref_person_id
              ...
              join tmp_version tv on (tv.version = nvl(fv.end_date, v_max_end_date) and tv.record_id = fv.record_id) -- Является актуальной версией                                                              
      ...
      from (
            ...
            select /*+ use_hash(t person) parallel(t 4) parallel(person 4)*/ t.id as person_id,
              ...
              from ndfl_person t join ref_book_person person on (replace(lower(nvl(person.last_name,'empty')),' ','') = replace(lower(t.last_name),' ','') and
                                 ...
                                 join tmp_version tv on (tv.version = nvl(person.end_date, v_max_end_date) and tv.record_id = person.record_id)  -- Является актуальной версией                                                               
                                 left join ref_book_id_doc doc on (doc.person_id in (select id from ref_book_person where record_id=person.record_id))
                                 left join ref_book_id_tax_payer tax on (tax.person_id in (select id from ref_book_person where record_id=person.record_id))

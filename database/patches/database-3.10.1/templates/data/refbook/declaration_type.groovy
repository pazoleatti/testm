 ...
 */
    ...
    def importNdflResponse() {
        ...
        declarationDataFile.setFileKind("(${findKndByFileName()}) ${findDocTypeByFileName()}")
        ...
        logFormAttachResponseEvent(declarationData.id, msg)
        logger.info("Создана запись в историю изменений для отчетной формы №$declarationData.id")
        if (isAutoUpload()) {
    ...
    String findDocTypeByFileName() {
        String fileName = UploadFileName
        String toReturn = ""
        if (fileName.toLowerCase().startsWith(NDFL2_PATTERN_PROT_1.toLowerCase()) ||
                fileName.toLowerCase().startsWith(NDFL2_PATTERN_PROT_2.toLowerCase())) {
            toReturn = "Протокол Приема 2НДФЛ"
        } else if (fileName.toLowerCase().startsWith(NDFL2_PATTERN_REESTR_1.toLowerCase()) ||
                fileName.toLowerCase().startsWith(NDFL2_PATTERN_REESTR_2.toLowerCase())) {
            toReturn = "Реестр Принятых Документов"
        } else if (fileName.startsWith("KV_NONDFL6")) {
            toReturn = "Квитанция о приеме налоговой декларации (расчета) в электронном виде"
        } else if (fileName.startsWith("UO_NONDFL6")) {
            toReturn = "Уведомление об отказе в приеме налоговой декларации (расчета) в электронном виде"
        } else if (fileName.startsWith("IV_NONDFL")) {
            toReturn = "Извещение о вводе"
        } else if (fileName.startsWith("UU_NONDFL6")) {
            toReturn = "Уведомление об уточнении"
        } else if (fileName.startsWith("TR_SOSH_")) {
            toReturn = "Сообщение об ошибке"
        }
        return toReturn
    }

    String findKndByFileName() {
        String fileName = UploadFileName
        String toReturn = ""
        if (fileName.toLowerCase().startsWith(NDFL2_PATTERN_PROT_1.toLowerCase()) ||
                fileName.toLowerCase().startsWith(NDFL2_PATTERN_PROT_2.toLowerCase())) {
...
//Параметры имени ТФ
    ...
    def _importTF() {
        ...
        List<DepartmentReportPeriod> departmentReportPeriods =
                departmentReportPeriodService.getPeriodsSortedByFormTypePriority(departmentId, reportPeriodCode, year, true)
        if (CollectionUtils.isEmpty(departmentReportPeriods)) {
            departmentReportPeriods = departmentReportPeriodService.getPeriodsSortedByFormTypePriority(
                    departmentId, reportPeriodCode, year, false)
        }

...
declare 
	v_count number;
begin
	select count(1) into v_count from user_tables where table_name='REF_BOOK_OKTMO_TMP3';
	dbms_output.put_line('Create table REF_BOOK_OKTMO_TMP3...');
	if v_count = 1 then
	   execute immediate 'DROP TABLE REF_BOOK_OKTMO_TMP3';
	end if;
	execute immediate 
		'CREATE TABLE REF_BOOK_OKTMO_TMP3 (
                      ...
                      razd      NUMBER(1) )';
		dbms_output.put_line('Table REF_BOOK_OKTMO_TMP3 created.');	
exception when others then
   dbms_output.put_line(sqlerrm);
   raise_application_error(-20999,'Error create table REF_BOOK_OKTMO_TMP3.');   
end;
/
...
--3.10.1-skononova-2
HOST "&2\sqlldr" &3 control=database-3.10.1/templates/ldr/refbook/oktmo.ldr log=&4/3_10_1_02_03_oktmo.txt bad=&5/3_10_1_02_03_oktmo.bad rows=1000
...
begin
	select count(1) into v_count from user_tables where table_name='REF_BOOK_OKTMO_TMP3';
	dbms_output.put_line('Drop table REF_BOOK_OKTMO_TMP3...');
	if v_count > 0 then 
		execute immediate 'drop table REF_BOOK_OKTMO_TMP3';
		dbms_output.put_line('Table REF_BOOK_OKTMO_TMP3 was dropped.');	
	end if;	

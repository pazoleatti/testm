--добавление поисковых полей в реестр физ. лиц
DECLARE
...
--добавление поисковых полей в налоговые формы
DECLARE
	v_run_condition number(1);
	v_task_name varchar2(128):='alter_tables block #8 - alter table NDFL_PERSON';  
BEGIN
	select decode(count(*),0,1,0) into v_run_condition from user_tab_columns where TABLE_NAME='NDFL_PERSON' and COLUMN_NAME='SEARCH_DOC_NUMBER';
	IF v_run_condition=1 THEN
	        execute immediate 'alter table NDFL_PERSON add  SEARCH_DOC_NUMBER VARCHAR2(25 CHAR)';
		dbms_output.put_line(v_task_name||'[INFO ]:'||' Success');
	ELSE
		dbms_output.put_line(v_task_name||'[WARNING]:'||' changes had already been implemented');
	END IF;
...
DECLARE
	v_run_condition number(1);
	v_task_name varchar2(128):='alter_tables block #9 - alter table NDFL_PERSON';  
BEGIN
	select decode(count(*),0,1,0) into v_run_condition from user_tab_columns where TABLE_NAME='NDFL_PERSON' and COLUMN_NAME='SEARCH_LAST_NAME';
	IF v_run_condition=1 THEN
	        execute immediate 'alter table NDFL_PERSON add SEARCH_LAST_NAME VARCHAR2(60 CHAR)';
		dbms_output.put_line(v_task_name||'[INFO ]:'||' Success');
	ELSE
		dbms_output.put_line(v_task_name||'[WARNING]:'||' changes had already been implemented');
	END IF;
...
DECLARE
	v_run_condition number(1);
	v_task_name varchar2(128):='alter_tables block #10 - alter table NDFL_PERSON';  
BEGIN
	select decode(count(*),0,1,0) into v_run_condition from user_tab_columns where TABLE_NAME='NDFL_PERSON' and COLUMN_NAME='SEARCH_FIRST_NAME';
	IF v_run_condition=1 THEN
	        execute immediate 'alter table NDFL_PERSON add SEARCH_FIRST_NAME VARCHAR2(60 CHAR)';
		dbms_output.put_line(v_task_name||'[INFO ]:'||' Success');
	ELSE
		dbms_output.put_line(v_task_name||'[WARNING]:'||' changes had already been implemented');
	END IF;
...
DECLARE
	v_run_condition number(1);
	v_task_name varchar2(128):='alter_tables block #11 - alter table NDFL_PERSON';  
BEGIN
	select decode(count(*),0,1,0) into v_run_condition from user_tab_columns where TABLE_NAME='NDFL_PERSON' and COLUMN_NAME='SEARCH_MIDDLE_NAME';
	IF v_run_condition=1 THEN
	        execute immediate 'alter table NDFL_PERSON add SEARCH_MIDDLE_NAME VARCHAR2(60 CHAR)';
		dbms_output.put_line(v_task_name||'[INFO ]:'||' Success');
	ELSE
		dbms_output.put_line(v_task_name||'[WARNING]:'||' changes had already been implemented');
	END IF;
...
DECLARE
	v_run_condition number(1);
	v_task_name varchar2(128):='alter_tables block #12 - alter table NDFL_PERSON';  
BEGIN
	select decode(count(*),0,1,0) into v_run_condition from user_tab_columns where TABLE_NAME='NDFL_PERSON' and COLUMN_NAME='SEARCH_INN';
	IF v_run_condition=1 THEN
	        execute immediate 'alter table NDFL_PERSON add SEARCH_INN VARCHAR2(12 CHAR)';
		dbms_output.put_line(v_task_name||'[INFO ]:'||' Success');
	ELSE
		dbms_output.put_line(v_task_name||'[WARNING]:'||' changes had already been implemented');
	END IF;
...
DECLARE
	v_run_condition number(1);
	v_task_name varchar2(128):='alter_tables block #13 - alter table NDFL_PERSON';  
BEGIN
	select decode(count(*),0,1,0) into v_run_condition from user_tab_columns where TABLE_NAME='NDFL_PERSON' and COLUMN_NAME='SEARCH_INN_FOREIGN';
	IF v_run_condition=1 THEN
	        execute immediate 'alter table NDFL_PERSON add SEARCH_INN_FOREIGN VARCHAR2(50 CHAR)';
		dbms_output.put_line(v_task_name||'[INFO ]:'||' Success');
	ELSE
		dbms_output.put_line(v_task_name||'[WARNING]:'||' changes had already been implemented');
	END IF;
...
DECLARE
	v_run_condition number(1);
	v_task_name varchar2(128):='alter_tables block #14 - alter table NDFL_PERSON';  
BEGIN
	select decode(count(*),0,1,0) into v_run_condition from user_tab_columns where TABLE_NAME='NDFL_PERSON' and COLUMN_NAME='SEARCH_SNILS';
	IF v_run_condition=1 THEN
	        execute immediate 'alter table NDFL_PERSON add SEARCH_SNILS VARCHAR2(14 CHAR)';
		dbms_output.put_line(v_task_name||'[INFO ]:'||' Success');
	ELSE
		dbms_output.put_line(v_task_name||'[WARNING]:'||' changes had already been implemented');
	END IF;

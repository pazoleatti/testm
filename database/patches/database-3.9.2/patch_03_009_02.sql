...
begin 
	...
	if :v_cnt = 0 then
		dbms_output.put_line('Error: 03.009.01 not installed. Check log-files.');
		raise_application_error(-20999,'Error: 03.009.01 not installed. Check log-files.');
	end if;

  	select count(1) into :v_cnt from version_history where status='OK' and version='03.009.02';
	if :v_cnt > 0 then
		dbms_output.put_line('Version 03.009.02 already installed.');
...
@database-3.9.2/01_ddl_tables_views_synonyms.sql &3 &6

PROMPT ## 02_templates
@database-3.9.2/02_templates.sql "_log/3.9_1_03_templates.txt" "&4" "&5" "../_log" "../_bad"

PROMPT ## 06_check_index_constraints
@database-3.9.2/06_check_index_constraints.sql

insert into version_history (version, scrname, status, start_time, end_time) values (:version, 'patch_03_009_02.sql', :status, to_timestamp(:start_time,'yyyy.mm.dd hh24:mi:ss:FF3'), systimestamp);
commit;

...
begin 
	...
	if :v_cnt = 0 then
  	...
  	select count(1) into :v_cnt from version_history where status='OK' and version='03.009.02.04';
	if :v_cnt > 0 then
		dbms_output.put_line('Version 03.009.02.04 already installed.');
		raise_application_error(-20999,'Version 03.009.04 already installed.');
	end if; 
end;
/  
...
PROMPT ## 02_templates
@database-3.9.2.4/02_templates.sql "_log/3.9_2_04_templates.txt" "&4" "&5" "../_log" "../_bad"

PROMPT ## 03_indexes
@database-3.9.2.4/03_indexes.sql

PROMPT ## 06_check_index_constraints
@database-3.9.2.4/06_check_index_constraints.sql

insert into version_history (version, scrname, status, start_time, end_time) values (:version, 'patch_03_009_02_04.sql', :status, to_timestamp(:start_time,'yyyy.mm.dd hh24:mi:ss:FF3'), systimestamp);
commit;

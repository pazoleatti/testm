...
class Calculate extends AbstractScriptClass {
    ...
    String snilsQuery = "with t as (select np.id, np.search_snils from ndfl_person np where declaration_data_id=:p_declaration and person_id is null) select person2.person_id, person3.id as ref_book_person_id from (select /*+ INDEX (person  IDX_REF_BOOK_PERSON_SRCH_SNIL1)*/ t.id as person_id, person.id as ref_book_person_id from  t join ref_book_person person on (person.search_snils = t.search_snils and person.start_date <= sysdate and ( (person.end_date is null) or (person.end_date >= sysdate) ) ) where  person.record_id=person.old_id) person2 join ref_book_person person3 on (person3.id=person2.ref_book_person_id)"

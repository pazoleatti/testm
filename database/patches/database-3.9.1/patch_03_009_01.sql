...
begin 
	...
	if :v_cnt > 0 then
		dbms_output.put_line('Version 03.009.01 already installed.');
		raise_application_error(-20999,'Version 03.009.01 already installed.');
	end if; 
end;
/  
...
insert into version_history (version, scrname, status, start_time, end_time) values (:version, 'patch_03_009_01.sql', :status, to_timestamp(:start_time,'yyyy.mm.dd hh24:mi:ss:FF3'), systimestamp);
commit;

...
begin 
	...
	if :v_cnt = 0 then
  	...
  	select count(1) into :v_cnt from version_history where status='OK' and version='03.007.fx.01';
	if :v_cnt > 0 then
		dbms_output.put_line('Hotfix 03.007.hotfix.01 already installed.');
	end if; 
end;
/  
...
insert into version_history (version, scrname, status, start_time, end_time) values (:version, 'patch_03_007_hotfix_01.sql', :status, to_timestamp(:start_time,'yyyy.mm.dd hh24:mi:ss:FF3'), systimestamp);
commit;

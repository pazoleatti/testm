...
begin 
	...
	if :v_cnt > 0 then
		dbms_output.put_line('Version 03.010.00 already installed.');
		raise_application_error(-20999,'Version 03.010.00 already installed.');
	end if; 
end;
/  
...
PROMPT ## 00_update-1
@database-3.10/00_update_dml.sql 


PROMPT ## 01_ddl_tables_views_synonyms
@database-3.10/01_ddl_tables_views_synonyms.sql &3 &6

PROMPT ## 02_templates
@database-3.10/02_templates.sql "_log/3.10_03_templates.txt" "&4" "&5" "../_log" "../_bad"

PROMPT ## 03_update-2
@database-3.10/03_update_dml.sql 

 
PROMPT ## 06_check_index_constraints
@database-3.10/06_check_index_constraints.sql

PROMPT ## get statistics
@database-3.10/08_get_statistics.sql

insert into version_history (version, scrname, status, start_time, end_time) values (:version, 'patch_03_010_00.sql', :status, to_timestamp(:start_time,'yyyy.mm.dd hh24:mi:ss:FF3'), systimestamp);
commit;

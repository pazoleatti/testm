...
using (select distinct t.razd,t.code,first_value(t.record_id) over(partition by t.razd,t.code order by t.id) first_rec_id
        from ref_book_oktmo t
       /*where exists(select 1 from ref_book_oktmo d where d.code=t.code and d.status=-1)*/) nr
 on (r.code=nr.code and r.razd=nr.razd)

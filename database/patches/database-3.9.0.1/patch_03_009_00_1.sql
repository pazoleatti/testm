...
begin 
	...
	if :v_cnt > 0 then
		dbms_output.put_line('Version 03.009.00.1 already installed.');
--		raise_application_error(-20999,'Version 03.009.00.1 already installed.');
	end if; 
end;
/  
...
PROMPT ## 02_templates
@database-3.9.0.1/02_templates.sql "_log/3.9.0.1_03_templates.txt" "&4" "&5" "../_log" "../_bad"

PROMPT ## 06_check_index_constraints
@database-3.9.0.1/06_check_index_constraints.sql


insert into version_history (version, scrname, status, start_time, end_time) values (:version, 'patch_03_009_00_1.sql', :status, to_timestamp(:start_time,'yyyy.mm.dd hh24:mi:ss:FF3'), systimestamp);
commit;

...
public class QRefBookNdfl extends com.querydsl.sql.RelationalPathBase<QRefBookNdfl> {
    ...
    public QRefBookNdfl(String variable) {
        super(QRefBookNdfl.class, forVariable(variable), "NDFL_1_0", "REF_BOOK_NDFL");
        addMetadata();
    }
    ...
    public QRefBookNdfl(Path<? extends QRefBookNdfl> path) {
        super(path.getType(), path.getMetadata(), "NDFL_1_0", "REF_BOOK_NDFL");
        addMetadata();
    }

    public QRefBookNdfl(PathMetadata metadata) {
        super(QRefBookNdfl.class, metadata, "NDFL_1_0", "REF_BOOK_NDFL");
        addMetadata();
    }

    public void addMetadata() {
        addMetadata(departmentId, ColumnMetadata.named("DEPARTMENT_ID").withIndex(2).ofType(Types.DECIMAL).withSize(18));
        addMetadata(id, ColumnMetadata.named("ID").withIndex(1).ofType(Types.DECIMAL).withSize(18).notNull());
        addMetadata(inn, ColumnMetadata.named("INN").withIndex(3).ofType(Types.VARCHAR).withSize(12));
        addMetadata(recordId, ColumnMetadata.named("RECORD_ID").withIndex(4).ofType(Types.DECIMAL).withSize(9).notNull());

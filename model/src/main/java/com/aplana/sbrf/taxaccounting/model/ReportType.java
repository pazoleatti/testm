...
public enum ReportType {
    ...
    IMPORT_FD(2, "IMPORT_FORM_DATA", "Импорт файла %s с формы экземпляра %sформы", 18L, 118L), //импорт из режима редактирования
    CHECK_FD(3, "CHECK_FORM_DATA", "Проверка %sформы", 0L, 0L),
    IMPORT_TF_FD(4, "IMPORT_TF_FORM_DATA", "Импорт файла %s для %sформы из каталога загрузки", 0L, 0L), //формально является задачей, блокирует форму при импорт из каталога загрузки

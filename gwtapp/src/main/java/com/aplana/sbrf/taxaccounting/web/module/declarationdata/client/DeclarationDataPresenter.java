...
public class DeclarationDataPresenter extends Presenter<DeclarationDataPresenter.MyView, DeclarationDataPresenter.MyProxy>
	...
	public void prepareFromRequest(PlaceRequest request) {
		...
		final long id = Integer.valueOf(request.getParameter(DeclarationDataTokens.declarationId, "0"));
		if (id != 0) {
			...
			dispatcher.execute(action, CallbackUtils
					.defaultCallback(new AbstractCallback<GetDeclarationDataResult>() {
						...
						public void onSuccess(GetDeclarationDataResult result) {
							...
							getView().showDownloadXml(result.isCanDownload());
	...
	public void downloadExcel() {
		Window.open(GWT.getHostPageBaseURL() + "download/declarationData/xlsx/" + declarationId, null, null);
	}

	@Override
	public void downloadXml() {
		Window.open(GWT.getHostPageBaseURL() + "download/declarationData/xml/" + declarationId, null, null);
	}

...
import com.aplana.sbrf.taxaccounting.web.module.formdata.shared.RefreshDataRowsAction;
...
/**
 * @author lhaziev Обработчик запроса для обновления формы.
 */
@Service
@PreAuthorize("hasAnyRole('ROLE_OPER', 'ROLE_CONTROL', 'ROLE_CONTROL_UNP', 'ROLE_CONTROL_NS')")
public class RefreshFormDataHandler extends AbstractActionHandler<RefreshDataRowsAction, TaskFormDataResult> {
	...
	public RefreshFormDataHandler() {
		super(RefreshDataRowsAction.class);
	}

	@Override
	public TaskFormDataResult execute(final RefreshDataRowsAction action,
			ExecutionContext context) throws ActionException {
        final ReportType reportType = ReportType.REFRESH_FD;
            ...
            if (restartStatus != null && restartStatus.getFirst()) {
                ...
                asyncTaskManagerService.createTask(keyTask, reportType, params, action.isCancelTask(), PropertyLoader.isProductionMode(), userInfo, logger, new AsyncTaskHandler() {
                    ...
                    public void interruptTask(ReportType reportType, TAUserInfo userInfo) {
                        formDataService.interruptTask(action.getFormData().getId(), false, userInfo.getUser().getId(), reportType, "Выполнено обновление формы");
                    }
	...
	@Override
	public void undo(RefreshDataRowsAction action, TaskFormDataResult result,
			ExecutionContext context) throws ActionException {
		// Ничего не делаем
	}
}

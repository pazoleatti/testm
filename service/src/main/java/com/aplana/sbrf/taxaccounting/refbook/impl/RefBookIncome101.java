...
public class RefBookIncome101 implements RefBookDataProvider {
    public final static long REF_BOOK_ID = 50;
    @Autowired
    ...
    public PagingResult<Map<String, RefBookValue>> getRecords(Date version, PagingParams pagingParams, String filter, RefBookAttribute sortAttribute) {
        ReportPeriod reportPeriod = getReportPeriod(version);
        return refBookIncome101Dao.getRecords(REF_BOOK_ID, reportPeriod.getId(), pagingParams, filter, sortAttribute);
    }
    ...
    public Map<String, RefBookValue> getRecordData(Long recordId) {
        return refBookIncome101Dao.getRecordData(REF_BOOK_ID, recordId);
    }
    ...
    public void insertRecords(Date version, List<Map<String, RefBookValue>> records) {
        ...
        ReportPeriod reportPeriod = getReportPeriod(version);
        refBookIncome101Dao.insert(REF_BOOK_ID, reportPeriod.getId(), records);
    }

    @Override
    public void updateRecords(Date version, List<Map<String, RefBookValue>> records) {
        refBookIncome101Dao.update(records);
    }
    ...
    public void deleteAllRecords(Date version) {
        ReportPeriod reportPeriod = getReportPeriod(version);
        refBookIncome101Dao.deleteAll(reportPeriod.getId());
    }

    @Override
    public RefBookValue getValue(Long recordId, Long attributeId) {
        return refBookIncome101Dao.getValue(REF_BOOK_ID, recordId, attributeId);
    }

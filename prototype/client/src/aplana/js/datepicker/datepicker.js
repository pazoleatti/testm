                        ...
                        controller.$parsers.unshift(function (viewValue) {
                            if (controller.$error.date) {
                                ...
                                if (angular.isDate(controller.$dateValue)) {
                                    ...
                                    today = new Date();
                                    if (scope.options.roundTime) {
                                        today = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0, 0);
                                    }
                                }

                                var isMinValid = isNaN(scope.options.minDate) || today.getTime() >= scope.options.minDate;
                                var isMaxValid = isNaN(scope.options.maxDate) || today.getTime() <= scope.options.maxDate;
